...
\item\label{item2} There exists an $N$-function $\Phi_1 \in \Delta_2$ globally???? such that
\begin{equation}
...
\Phi(rs)\geq k r^\nu \Phi(s)\;\;r\geq 1,\;s>0,
\]
which is \eqref{} with $\Phi_1(r)=kr^\nu$ that is an $N$-function that satisfies the $\Delta_2$-condition globally.
...
\]
Let $u=\Phi_1(r)\geq \Phi_1(1)$ and $v=\Phi(s)\geq 0$. By the well known inequality \cite[(), p. ]{KR} we have  
\[
\frac{uv}{\Psi^{-1}(uv)}\leq \Phi^{-1}(uv)\leq\Phi_1^{-1}(u)\Phi^{-1}(v)\leq
\frac{4uv}{\Psi_1^{-1}(u)\Psi^{-1}(v)},\;\;u\geq \Phi_1(1),\;v\geq 0,
\]
then 
\[
...
\[
\Psi(\frac{xy}{4})\leq \Psi_1(x)\Psi(y).
\]
...
\begin{lem}
Let $\Phi,\Phi_2$ be $N$-functions and suppose that $\Phi \in \Delta_2$.
If $\Phi(rs)\geq \Phi_1(r)\Phi(s)$ for every $r\geq 1$ and $s>0$, then
\[
\lim\limits_{\|u\orlnor\to \infty}
\frac{\int_0^T \Phi(|u|)\,dt}{\Phi_0(\|u\orlnor)}=\infty,
\]
for every $\Phi_0$ with $\Phi_0=o(\Phi_1)$ at $\infty$.
\end{lem}
...
\]
Now, we choose $r=\frac{\|u\orlnor}{2}$, we use the fact that $\Phi_1\in\Delta_2$ and
$\Phi_0=o(\Phi_1)$ at $\infty$, and 
therefore we get
\[
\frac{\int_0^T \Phi(|u|)\,dt}{\Phi_0(\|u\orlnor)}\geq
\lim\limits_{\|u\orlnor \to \infty} \frac{\Phi_1(\frac{\|u\orlnor}{2})}{\Phi_0(\|u\orlnor)}
\geq
...
El siguiente lema debe {\bf adaptarse} a la nueva versi\'on $\Phi_0$.
...
\begin{lem}\label{lem_coer-version-phi} Let $\Phi$ and $\Psi$ be complementary $N$-functions and let $\b{u}\in\lphi$. Then:
\begin{enumerate}
  \item\label{th:coer_item1} $\|\b{u}\orlnor=O\left( \rho_{\Phi}\left(\b{u} \right)  \right)$ REVISAR!!!.
  
  \item\label{th:coer_item2} If $\Psi \in \Delta_2$ globally, then
	%then there exists a constant $\alpha_{\Phi}>1$ such that, for any $0<\mu<\alpha_{\Phi}$,
\begin{equation}\label{coer_modular} \Phi_0(\|\b{u}\orlnor) =o\left(\rho_{\Phi}\left(\b{u}\right)\right)
\end{equation}
as $\|\b{u}\orlnor\to \infty$.
being $\Phi_0 \in \Delta_2$ globally that satisfies 
$\Phi_0=o(\Phi_1)$ at $\infty$ and where $\Phi(rs)\geq \Phi_1(r)\Phi(s)$.

\item\label{th:coer_item3} If \eqref{coer_modular} holds for $\mu\geq 1$ then $\Psi \in \Delta_2$. REVISAR!!!!  
\end{enumerate}
\end{lem}

\begin{proof}
HACER LA PRUEBA!!!! DE AL MENOS EL SEGUNDO ITEM!!!!
...
%and $f(\b{x})=\varepsilon(\b{x})|\b{x}|^{\alpha_{\Phi}-1}$ which is  non-decreasing, sub additive and  
such that
