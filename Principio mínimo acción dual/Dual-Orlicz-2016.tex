...
\[
|u_n(t)-u(t)|\leq C\|u_n-u\orlnor,
\]
with $C=C(T)>0$. Then $u_n \to u$ uniformly.
As $u'_n\rightarrow u'\in\domi_{\Lambda}$, by 
  Lemma \ref{segundo lema}, there exist a subsequence of  $u'_{n_k}$ (again denoted $u'_{n_k}$) and a function  
	$h\in L^1([0,T],\rr)$
	such that  $u'_{n_k}\rightarrow u' \quad\text{a.e.}$ and $\Phi(u'_{n_k})\leq h\quad\text{a.e}$.  
...
|D_{x}\mathcal{L}(\cdot,u_{n_k},u'_{n_k})|\leq a(|u_{n_k}|)(b+\Phi(u'_{n_k}))\leq
M (b+h) \in L^1.
\end{equation*}
