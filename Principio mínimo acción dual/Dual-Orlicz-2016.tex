...
On the other hand, if $v'\in\lphi$ and $|s|\leq s_1 \lambda_v^{-1}$,
from the convexity and the parity of $\Phi$, we get
\[
\begin{split}
\Phi(u'+sv')&=
\Phi\left((1-s_1)\frac{u'}{1-s_1}+s_1 \frac{s}{s_1}v'\right)\\
&
\leq
(1-s_1)\Phi\left(\frac{u'}{1-s_1}\right)+s_1 \Phi\left(\frac{s}{s_1}v'\right)
\\
&\leq
(1-s_1)\Phi\left(\frac{u'}{1-s_1}\right)+s_1 \Phi\left(\frac{v'}{\lambda}\right)
\in L^1
\end{split}
\]
As $u'\in\Pi(\ephi,\Lambda)$ then
\[
d\left(\frac{u'}{1-s_1},E^{\Phi}\right)=\frac{1}{1-s_1}d(u', E^{\Phi})<1
\]
