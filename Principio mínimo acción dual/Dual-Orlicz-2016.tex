...
Let $\Phi_1$ and $\Phi_2$ be   $N_{\infty}$ functions. Following to \cite{trudinger1974imbedding} we wrtite $\Phi_1\strictif\Phi_2$ if there exists non negative numbers $k$ and $r_0$ such that
\begin{equation}\label{eq:orden} \Phi_1(x)\leq\Phi_2(kx),\quad |x|\geq r_0.\end{equation}
For example if $\Phi$ is $\Delta_2$ then there exist $p\in (1,+\infty)$ such that $\Phi\strictif |x|^p$.  If for every $k>0$ there exists $r_0=r_0(k)>0$ such that \eqref{eq:orden} holds we write  $\Phi_1\llcurly\Phi_2$. 


If a Lagrange functio $\mathcal
...
Consequently if $\Psi$ satisfies the $\Delta_2^{\infty}$-condition then $\lphi\left([0,T],\rr^d\right)$ can be equipped with the weak* topology.
...
\begin{example} Let $\Phi=\Phi_{p_1,p_2}$ be the function given in Example \eqref{ex:pq-laplac}.  We show that 
 \[
 K_1\min\left\{\frac{r^{p_1}}{p_1}, \frac{r^{p_2}}{p_2}\right\}\leq A_{\Phi}(r)\leq \min\left\{\frac{r^{p_1}}{p_1}, \frac{r^{p_2}}{p_2}\right\}
\]
...
\begin{equation}\label{eq:aphi}\Phi_{p_1,p_2}(y_1,y_2)\geq K\min\left\{\frac{r^{p_1}}{p_1}, \frac{r^{p_2}}{p_2}\right\},\end{equation}
with $K=\min\{2^{-p_1},2^{-p_2}\}$. 
