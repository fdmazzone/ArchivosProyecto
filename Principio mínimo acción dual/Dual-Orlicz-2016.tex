...
The coercivity of a Young's function $\Phi$ implies that 
%the existence of a function $F:\rr^+\to \rr^+$
 for every $K>0$ there exists a function $F:=F(K)$ 
such that if $|x|>F(K)$ then $\Phi(x)> K$.

{\color{red} Con menores estrictos????}

It is possible to define a function $F$ that satisfies the property above as follows
\begin{equation}\label{eq:inversa-gral}
F(s)=\sup\{|x|:\Phi(x)\leq s\},
\end{equation}
where $\Phi$ is a coercive Young's function.

As $\alpha \Phi(\frac{x}{\alpha})$ is non decreasing with respect to $\alpha$, 
we get that the function $\alpha F(\frac{x}{\alpha})$ is also increasing with respect to $\alpha$. That is,
...
It is easy to see that  $|x|\leq F(\Phi(x))$; 
and, if  $\Phi$ is a scalar function, then $F=\Phi^{-1}$.

{\color{red} No me convence como est\'a escrito lo anterior porque no s\'e 
c\'omo referenciarlo cuando lo aplicamos para obtener las desigualdades.}
...
\begin{lem}\label{inclusion orlicz} Let $\Phi:\rr^d\to [0,+\infty)$ be a Young's function and let $u\in\wphi([0,T],\rr^d)$ . 

{\color{red} Let $F: \rr^+ \to \rr^+$ be the function defined by \eqref{eq:inversa-gral}.}
\begin{enumerate}
...
\\
& \| u\linf \leq 2F\left(\frac{1}{T}\right)\max\{1,T\}\|u\sobnor\tag{Sobolev's inequality}\label{sobolev}
\end{align}
...
\[
 ...
 \end{split}
\]
{\color{red} As $\Phi$ is coercive, by (quiz\'as referencia a la intro???) there exists a function $F:\rr^+\to \rr^+$ such that}
\[
...
Taking into account that $\alpha\Phi(x/\alpha)$ is a non increasing function with respect to $\alpha\in [0,\infty)$ for every $x\in\rr^d$ we have
\[\Phi\left( \frac{u(t)-u(s)}{\|u'\orlnor T}\right)\leq \frac{1}{T}.\]
Dividing by $T$ this inequality, integrating respect to $s$ and using Jensen's inequality again
...
Then, {\color{red} by \ref{} there exists $F:\rr^+\to \rr^+$ such that}
\[\frac{|u(t)-\overline{u}|}{\|u'\orlnor T} \leq F\left(\frac{1}{T}\right),\]
and Sobolev-Wirtinger's inequality follows easily.

For the Sobolev's inequality we note that (we write $T^*=\max\{1,T\}$)
...
Using Jensen's inequality and that $T^*\geq 1$ we have
...
\]
Then $I_1+I_2\leq 1/T$. 
Now, {\color{red} by \ref{} there exists a function $F:\rr^+\to \rr^+$ such tha} 
\[
\frac{|u|}{2T^*\|u\sobnor}\leq F\left(\frac{1}{T}\right)
\]
and we obtain \ref{sobolev}  immediately.
