...
This topic was deeply addressed for the several types of \emph{Lagrange functions}. 
...
\right.
\]
These equations can be solved, among others, by the following sets (if $n>1$ infinite) of citical values: 
...
In order to employ second order conditions for constrained problems,
we define the tangent plane at the point $(x,y)\in \rr^{2n}$ as 
$M=\{(z,w) \in \rr^{2n}: zx^T+wy^T=0\}$.
%And, we are interested in knowing if the 
%matrix of second partial derivatives 
%$D^2 \Phi+ \lambda D^2 H$
%, with respect to the vector $(x,y)\in  \rr^{2n}$, 
%of 
We also consider the Lagrangian $\mathcal{L}(x,y,\lambda)=\Phi(x,y)+\lambda H(x,y)$ being $H=0$ the constraint.
And, we are interested in knowing how to behave the matrix 
of second partial derivatives of $\mathcal L$
with respect to the vector $(x,y)\in  \rr^{2n}$, given by $\mathcal{H}=D^2 \Phi+ \lambda D^2 H$,
on the subspace $M$.

 
We have to split the analysis in several cases according to the values of $p_1$ and $p_2$.

First, we deal with $p_1\leq 2$ and $p_2\leq 2$ being one of them different from 2.

We compute
\[(z,w)\mathcal{H}(z,w)^T=
%(z,w) (D^2\Phi+\lambda D^H) (z,w)^T=
|\lambda| (zx^T)^2 [|x|^{-2})(p_1-2)+(p_2-2)|y|^{-2}],\]
on the subspace $M$. 

As there exists $(z,w)=(-y,x)$  such that $zx^t+wy^t=0$, we obtain that

$(-y,x)\mathcal{H}(-y,x)^T=|\lambda||y|^2|x|^2[(p_1-2)|x|^{-2}+(p_2-2)|y|^{-2}]<0.$
...
PEDIMOS $\Psi \in \Delta_2$ de entrada y eliminamos la parte de la demicontinuidad????
...
Then, $\Psi(\frac{v_n-v}{\lambda})\leq c(|\lambda|)\Psi(v_n-v)$ for every $\lambda \in \rr$.

Therefore, $\Psi(\frac{v_n-v}{\lambda})\to 0$ a.e. as $n \to \infty$ and 
$\Psi(\frac{v_n-v}{\lambda})\leq c(|\lambda|K\Psi(v_n)+\Psi(v))
\leq c(|\lambda| K [m(t)+\Psi(v)])\in L^1$.
