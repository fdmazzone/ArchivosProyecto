...
\begin{proof}
Let $r:=d({u},\ephi)<1$. As ${u}_n$ converges to ${u}$, there exists a subsequence (again denoted $u_n$ such that $u_n\to u$ a.e. and $d(u_n,u)<(1-r)/2^{n+1}$. We define the numbers 
\[\lambda_1:=d(u_1,\ephi)<\frac{1+r}{2},\quad   \lambda_n=\|{u}_{n}-{u}_{n-1}\orlnor<\frac{1-r}{2^{n-1}},\quad\hbox{for } n\geq 2.\]
We take $v\in\ephi$ such that $d(u_1,v)<(1+r)/{2}$ and define 
Let $h:[0,T]\rightarrow\mathbb{R}$ by
\begin{equation}\label{eq:serie} h(x)= \lambda_1\Phi(\frac{v}{\lambda_1})+\Phi(\frac{u_1-v}{\lambda_1}   |{u}_{n_1}(x)|+\sum_{k=2}^{\infty}|{u}_{n_k}(x)-{u}_{n_{k-1}}(x)|.
\end{equation}
As a consequence  of \cite[Lemma 10.1]{KR} (see \cite[Thm. 5.5]{Orliczvectorial2005} for vector valued functions),  we have that $d({v},\ephi_d)=d(|{v}|,\ephi_1)$ for any ${v}\in\lphi_d$. 
Now
\[d(|{u}_{n_1}|,\ephi_1)= d({u}_{n_1},\ephi_d)\leq d({u}_{n_1},{u})+d({u},\ephi_d)<\frac{\lambda+r}{2}.\]
Then
\[d(h,\ephi_1)\leq d(h,|{u}_{n_1}|)+d(|{u}_{n_1}|,\ephi_1)< \lambda.\]
Therefore, $h\in\Pi(\ephi_1,\lambda)$ and  $|h|<\infty$ a.e. 
We conclude that the series  ${u}_{n_1}(x)+\sum_{k=2}^{\infty}({u}_{n_k}(x)-{u}_{n_{k-1}}(x))$
is absolutely convergent a.e. and this fact implies that ${u}_{n_k}\rightarrow {u} \quad\text{a.e.}$ 
The inequality $|{u}_{n_k}|\leq h$ follows straightforwardly from the definition of $h$.
\end{proof}
