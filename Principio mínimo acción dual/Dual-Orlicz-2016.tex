...
Let us take a moment to show that condition \eqref{eq:cond-sub} encompasses the sublinearity condition, for example, as it is formulated in \cite{li2014periodic} for the $(q,p)$-Laplacian. In \cite[Th. 1.1.]{li2014periodic} Li, Ou and Tang considered a potential $F:[0,T]\times\rr^d\times\rr^d\to\rr$ satisfying \ref{item:condicion_c} and \ref {item:condicion_a} and the condition \ref{item:condicion_h1} below. We write
$p'=p/(p-1)$ for the Lebesgue conjugate exponent of $p$.
\begin{enumerate}
 \item[(H1)]\label{item:condicion_h1} Existen $f_i,g_i,h_i\in L^1([0,T],\rr_+)$, $\alpha_1\in [0,q-1)$, $\alpha_2 \in [0,p-1)$, $\beta_1\in [0,p/q')$ and $\beta_2\in [0,q/p')$ such that
 \begin{equation*}
  \begin{split}
 ...
 Let us prove that \eqref{item:condicion_h1}$\Rightarrow$\ref{eq:cond-sub} where
 $\Phi(x_1,x_2)=\Phi_{q,p}=|x_1|^q/q+|x_2|^p/p$ and $
