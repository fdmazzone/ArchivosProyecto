...
Periodic Solutions, \sep Orlicz Spaces, \sep  Euler-Lagrange,  \sep  Critical Points.   
    }%
    \endgroup
%
%
...
\right. \tag{$P$}
\end{equation}
where the function $\mathcal{L}:[0,T]\times\rr^d\times\rr^d\to\rr$, $d\geq 1$ (hereinafter called the \emph{Lagrange function} or \emph{lagrangian}) satisfying  that  it is measurable in $t$ for each $(x,y)\in \rr^d\times\rr^d$ and  continuously differentiable in $(x,y)$ for almost every $t \in [0,T]$. The unknown function  $u:[0,T]\to\rr^d$ is assumed absolutely continuous. 
...
$\Phi:\mathbb{R}^d\to [0,+\infty)$ is a \emph{$N_{\infty}$ function} if $\Phi$ is convex, $\Phi(0)=0$, $\Phi(y)>0$ if $y\neq 0$, $\Phi(-y)=\Phi(y)$,
 and 
...
Some examples of  $N_{\infty}$ functions are the following.
  ...
  \begin{example}\label{ex:p-laplac}
    $\Phi_p(y):=|y|^p/p$, for $1<p<\infty$.  In this case $\Phi^{\star}(\zeta)=|\zeta|^{p'}/{p'}$, $p':=p/(p-1)$.
  \end{example}

   \begin{example}
     If $\Phi:\rr\to [0,+\infty)$ is a $N_{\infty}$  function on $\rr$ then $\overline{\Phi}(y)=\Phi(|y|)$ is a $N_{\infty}$  function on $\rr^d$. In this example, as in the previous one, the function $\Phi$ is \emph{radial}, i.e. the value of $\Phi(y)$ depends only on the norm of $y$. These cases are not authentically anisotropic.
   \end{example}
...
Then $\Phi_{p_1,p_2}$ is a $N_{\infty}$ function.  In this case the complementary function is the function $\Phi_{p_1',p'_2}$. 

More generally, if $\Phi_k:\rr^d\to [0,+\infty)$, $k=1,\ldots,n$, are $N_{\infty}$ functions, then $\Phi:\rr^d\times\cdots\times\rr^d\to [0,+\infty)$ defined by $\Phi(y_1,\ldots,y_n)=\Phi_1(y_1)+\cdots+\Phi_n(y_n)$  is a $N_{\infty}$ function.
   \end{example}

 \begin{example}
      When $\Phi$ does not satisfy the $\Delta_2$ condition (see \eqref{delta2defi} below), an anisotropic $N_{\infty}$  function is not necessarily controlled by powers functions. For example $\Phi:\rr^d:\to[0,+\infty)$ defined by $\Phi(y)=\exp(|y|)-1$ is $N_{\infty}$ function.
    \end{example}
    ...
    We will find solutions of \eqref{ProbPrin-gral} for finding extreme points of the \emph{action integral} 
\begin{equation}\label{eq:integral_accion}
  I(u)=I_{\mathcal{L}}(u):=\int_{0}^T \mathcal{L}(t,u(t),u'(t))\ dt.\tag{$IA$}
\end{equation}

   


In order that $I$ is defined and be differentiable over certain Sobolev-Orlicz spaces (which we will define later) we need to introduce the following property.
...
\end{equation}
Here $\Phi$ is assumed differentiable and  $F(t,x)$, which is often referred to potential,  is differentiable with respect to $x$ for a.e. $t\in [0,T]$. Moreover, it is usually assumed that $F$ satisfies the following conditions:
\begin{enumerate}
