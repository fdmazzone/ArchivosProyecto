...
The lagrangian $\mathcal{L}_{\Phi,F}$ satisfies condition  \eqref{eq:condicion-estructura}. In order to prove this, the only non trivial fact that we should to establishis is that $ \Phi^{\star}(\nabla_{y}\mathcal{L})
\leq
...
\begin{enumerate}
  \item\label{it:prop1} $A_{\Phi}$ is continuous and $A_{\Phi}(s)>0$, when $s>0$,
...
Note that $A_{\Phi}$ is not  necessarily convex.  In some applications we overcome this lack by taking the \emph{greatest convex minorant } of $A_{\Phi}$. This function is defined as follows

\begin{equation}\label{eq:fun_bphi} B_{\Phi}(s)=\sup\{\varphi(s)| \varphi\text{ es convex and } \varphi\leq A_{\Phi}\}.
\end{equation}
The function $B_{\Phi}$ is the greatest convex radial minorant of $\Phi$. Moreover $B_{\Phi}$ is $N^{\infty}$ function.  To justify this statement it would only be necessary to prove that $B_{\Phi}(s)>0$, when $s>0$ and  $B_{\Phi}(s)/s\to\infty$. We write, for $x\in\rr$, $x^+=\max\{x,0\}$. From Proposition \ref{prop:AsubPhi}\eqref{it:prop2} we obtain that $A_{\Phi}(s)\geq A_{\Phi}(s_0)(s-s_0)^+/s_0>0$, for $s>s_0$.   From this it follows that $B_{\Phi}(s)\geq A_{\Phi}(s_0)(s-s_0)^+/s_0>0$, when $s>0$ and that $\liminf_{s\to\infty} B_{\Phi}(s)/s\geq A_{\Phi}(s_0)/s_0$. Hence, from   Proposition \ref{prop:AsubPhi}\eqref{it:prop2} we obtain that $B_{\Phi}(s)/s\to\infty$.
...
\end{comentario}
 \begin{cor}\label{cor:unif_conv} Every bounded sequence $\{\b{u}_n\}$ in  $\wphi([0,T],\rr^d)$  has an uniformly convergent subsequence. 
\end{cor}
...
\nabla_y\mathcal{L}(\cdot,u,u')\in \lambda C^{\Phi^{\star}}\left([0,T],\rr^d\right)\subset\lpsi\left([0,T],\rr^d\right)\subset\left[\lphi\left([0,T],\rr^d\right)\right]^*.
\end{equation}
...
\end{equation}
Let $M$ be a weak* closed subspace of $\lphi$. 
Then  $I$ attains a minimum on $H=\{u\in \wphi:u'\in M\}$.
\end{thm}

CHEQUEAR DONDE SE MINIMIZAR\'A, PERI\'ODICAS O NO????
...
\emph{Step 2. The action integral $I$ is sequentially weak* lower semicontinuous, i.e. if  $u_n'\rightharpoonup u'$ in the weak* topology of $ \wphi([0,T],\rr^d)$ then
$
...
Assume that $u_n\rightharpoonup u$ in  $ \wphi([0,T],\rr^d)$.  Without loss of generality, passing to subsequences, we may assume that the $\liminf$ is really a $\lim$ and, using Corollary \eqref{cor:unif_conv},  that  $u_n\to u$ uniformly and $u_n'\rightharpoonup u'$ in  $ \lphi([0,T],\rr^d)$. The embedding  $\lphi([0,T],\rr^d)\hookrightarrow L^1([0,T],\rr^d)$ implies that  $u_n'\rightharpoonup u' $ in $ L^1([0,T],\rr^d)$.  Now, applying \cite[Th. 3.6]{buttazzo1998one} we obtain $I(u)\leq \lim_{n\to\infty}I(u_n)$.

\emph{Final step}
The proof of the theorem is concluded with a usual argument. We take a minimizing sequence $u_n \in H$ of $I$. 
From the coercivity of $I$ we have that $u_n$ is bounded in $\wphi([0,T],\rr^d)$. By Corollary \ref{cor:unif_conv}  we can suppose that $u_n$converges uniformly to a function $u$. 
On the other hand, $u'_n$ is bounded in $\lphi=\left[\epsi\right]^*$. Thus,  
