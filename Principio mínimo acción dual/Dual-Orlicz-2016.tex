...
\end{equation}
for every $x$.  
We note that the $\Delta_2^{\infty}$ implies the following form of quasi-subadditivity. There exists $K>0$ such that for every $x,y\in \rr^d$
\begin{equation}\label{eq:quasi-sub-aditividad}
\Phi(x+y)\leq K (\Phi(x)+\Phi(y))+1.
\end{equation}

In addition, if $\Phi \in \Delta_2^{\infty}$, it holds that for any $\lambda>0$ there exists $c:\rr^+\to \rr^+$ such that 
\begin{equation}\label{eq:delta2-lambda}
\Phi(\lambda x)\leq c(\lambda)\Phi(x)+1. 
\end{equation}


If $\Phi$ is a $\Delta_2^{\infty}$ function, then $\Phi$ is bounded by powers functions (see \cite[Proof Lemma 2.4]{Desch2001} and \cite[Prop. 1]{cianchi2000local}), i.e. there exists $1<p<\infty$, $C>0$ and $r\geq 0$ such that
...
\\
 &\leq 
%M \left\{\left[ b(t)+ \Phi\left(\frac{u'+sv'}{\Lambda}\right)\right]|v|\right\}+
 %\lambda\lambda_v M[ b(t)+ \Phi\left(\frac{u'+sv'}{\Lambda}\right)]+\lambda \lambda_v \Phi\left(\frac{v'}{\lambda_v}\right)
 %\\
 %&=
