...
\begin{equation}\label{eq:nabla2}
  \Phi(x)\leq \frac{r}{\lambda}\Phi(\lambda x)+C.
\end{equation}
...
\end{split}
\end{equation}
where $C_2=C_2(\|d\|_{L^1})$ and $C_1=C_1(\|d\|_{L^1},\lambda)$.  From \eqref{eq:nabla2}
...
\emph{Step 2. If $u_n\to u$ uniformly and  $u_n'\rightharpoonup u'$ in $ \lphi([0,T],\rr^d)$ then
$
I(u)\leq\liminf_{n\to\infty}I(u_n)
$.
}  

 From the embedding $\lphi([0,T],\rr^d)\hookrightarrow L^1([0,T],\rr^d)$ we have the  $u_n'\rightharpoonup u' $ in $ L^1([0,T],\rr^d)$.  
