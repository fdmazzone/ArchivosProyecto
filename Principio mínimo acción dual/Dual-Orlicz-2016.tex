...
We note that the function $\alpha F(\frac{x}{\alpha})$ is increasing with respect to $\alpha$. That is,
if $\alpha \leq \beta$, we have 
\[
\begin{split}
\alpha F\left(\frac{s}{\alpha}\right)=
\sup\left\{|x|:\Phi(x)\leq \frac{s}{\alpha}\}=\sup\{\alpha|x|:\alpha\Phi(x)\leq s\right\}=
\\
\sup\left\{|y|:\alpha\Phi\left(\frac{y}{\alpha}\right)\leq s\}\leq \sup\{|y|:\beta\Phi\left(\frac{y}{\beta}\right)\leq s\right\}=
\\
...
It is easy to see that  $|x|\leq F(\Phi(x))$ and if  $\Phi$ is a scalar function, then $F=\Phi^{-1}$.
...
\[
 ...
 \end{split}
\]
As $\Phi$ is coercive, by (quiz\'as referencia a la intro???) there exists a function $F:\rr^+\to \rr^+$ such that 
\[
...
\ref{in-sob-cont}  implies \ref{wirtinger} by the following argument.  Taking into account that $\alpha\Phi(x/\alpha)$ is a non increasing function with respect to $\alpha\in [0,\infty)$ for every $x\in\rr^d$ we have
...
Then, by \ref{} there exists $F:\rr^+\to \rr^+$ such that 
...
Now, by \ref{} there exists a function $F:\rr^+\to \rr^+$ such that 
\[
