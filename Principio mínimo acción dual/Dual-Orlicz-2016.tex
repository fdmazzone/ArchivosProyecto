...
\begin{proof}
  Since $d({u},\ephi)<\lambda$ and ${u}_n$ converges to ${u}$, there exists   a subsequence of $u_n$ (again denoted $u_n$), $u_0\in\ephi$ and $0<r<\lambda$  such that $d(u_n,u_0)<r$, $n=1,\ldots$. Let $ \overline{\lambda}\in (r,\lambda)$.  As a consequence of \eqref{eq:N-sub-inf}, we obtain that $\lphi\left([0,T],\rr^d\right)\hookrightarrow L^1\left([0,T],\rr^d\right)$. This fact impies that $u_n$ converges in measure to $u$. Therefore, we can to extract a subsequence (denoted $u_n$) such that $u_n\to u$ a.e. and
  \[\lambda_n:=\|{u}_{n+1}-{u}_{n}\orlnor<\frac{\lambda-\overline{\lambda}}{2^n},\quad\hbox{for } n\geq 1.\]
We can assume $\lambda_n>0$ for every $n=1,\ldots$.

Let $\lambda_0:=\lambda-\sum_{n=1}^{\infty}\lambda_n$ and define $h:[0,T]\rightarrow\mathbb{R}$  by
\begin{equation}\label{eq:serie} h(t)= \frac{\lambda_0}{\lambda}\Phi\left(\lambda\frac{u_0}{\lambda_0}\right)+\sum_{n=1}^{\infty}\frac{\lambda_n}{\lambda}\Phi\left(\lambda\frac{u_{n+1}-u_n}{\lambda_n}\right).
\end{equation}
From the convexity and continuity of $\Phi$ we have for any $n=1,\ldots$


\[
 \begin{split}
   \Phi(u_n) &=\Phi\left(  u_0+   \sum_{j=0}^{n-1}u_{j+1}-u_j \right)\\
   &\leq
   \frac{\lambda_0}{\lambda}\Phi\left(\lambda\frac{u_0}{\lambda}\right)+\sum_{j=0}^{n-1}\lambda_j\Phi\left(\frac{u_{j+1}-u_j}{\lambda_j}\right) \leq h
 \end{split}
\]
\end{proof}
...
In this direction, the following theorem is our main result. The proof of it follows the same lines as \cite[Th. 3.2]{ABGMS2015} but with some modifications by the lack of monotony of $\Phi$ with  respect to the euclidean norm and the fact that we do not have of the notion of absolutely continuous norm.
...
\langle  I'(u),v\rangle= \int_0^T \left\{D_{x}\mathcal{L}\big(t,u,u'\big)\ccdot v
+ D_{y}\mathcal{L}\big(t,u,u'\big)\ccdot v'\right\} \ dt.
\end{equation}
...
Now, from  \eqref{eq:condicion-estructura} and Lemma \ref{lem:cota-a} we have
 \begin{equation}\label{eq:cota-condicion-estructura}
   \begin{split}
|\mathcal{L}(t,u(t),u'(t))|&+ |D_{x}\mathcal{L}(t,u(t),u'(t))|
+\Psi\left(\frac{D_{y}\mathcal{L}(t,u,u')}{\lambda}\right)
\\
...
\noindent\emph{Step 1. The non linear operator  $u \mapsto D_{x}\mathcal{L}(t,u,u')$ is continuous from $\domi_{\Lambda}$ into $L^{1}([0,T])$ with the strong topology on both sets.} 
...
As $u'_n\rightarrow u'\in\domi_{\Lambda}$, by 
  Lemma \ref{segundo lema}, there exist a subsequence of  $u'_{n}$ (again denoted $u'_{n}$) and a function  
	$h\in L^1([0,T],\rr)$
	such that  $u'_{n}\rightarrow u' \quad\text{a.e.}$ and $\Phi(u'_{n})\leq h\quad\text{a.e}$.  
...
|D_{x}\mathcal{L}(\cdot,u_{n},u'_{n})|\leq a(|u_{n}|)(b+\Phi(u'_{n}))\leq
M (b+h) \in L^1.
\end{equation*}
On the other hand, by the continuous differentiability of $\mathcal{L}$, we have
\[D_{x}\mathcal{L}(t,u_{n_k}(t),u'_{n_k}(t))\to D_{x}\mathcal{L}(t,u(t),u'(t))\quad\hbox{ for a.e. } t\in[0,T].\]
Applying the Dominated Convergence Theorem we conclude the proof of step 1.


\noindent\emph{Step 2. The non linear operator   $u
 \mapsto  D_{y}\mathcal{L}(t,u,u')$ is continuous from $\domi_{\Lambda}$ with the strong topology  
...
%\Psi(D_y\mathcal{L}(\cdot,u,u'))\leq a(|u|)(b+\Phi(u'))\in L^1
%\end{equation}
%then
\begin{equation}\label{eq:DyLpsi-clase}
D_{y}\mathcal{L}(\cdot,u,u')\in C^{\Psi}.
\end{equation}
...
We must prove that  $D_{y}\mathcal{L}(\cdot,u_n,u'_n)\overset{w^*}{\rightharpoonup} 
D_{y}\mathcal{L}(\cdot,u,u')$. 
...
\begin{equation}\label{cota_eps}
 \left| \langle D_{y}\mathcal{L}(\cdot,u_n,u'_n),v \rangle - 
\langle  D_{y}\mathcal{L}(\cdot,u,u'),v \rangle\right|\geq \epsilon.
\end{equation}
...
$u_n\rightarrow u$ uniformly, $u'_n\rightarrow u' \quad\text{a.e.}$ and $\Phi(u'_n)\leq h\quad\text{a.e.}$ 
...
\begin{equation}\label{eq:Dy-suc}
  \Psi(D_{y}\mathcal{L}(\cdot,u_n,u'_n))   
	\leq M (b+h)\in L^1.
\end{equation}
...
\begin{split}
&\lambda D_{y}\mathcal{L}(\cdot,u_{n_k},u'_{n_k})\ccdot \frac{v(t)}{\lambda} 
\\
&
\leq 
\lambda\left[\Psi(D_{y}\mathcal{L}(\cdot,u_{n_k},u'_{n_k}))+\Phi\left(\frac{v}{\lambda}\right)\right]
\\
...
\int_0^T  D_{y}\mathcal{L}(t,u_{n_k},u'_{n_k})
\ccdot  v \,dt 
\to 
\int_0^T D_{y}\mathcal{L}(t,u,u')\ccdot v\, dt \end{equation}
...
For  $|s|\leq s_0:=\min\{\left(1-d(u',\ephi)\right)/\|v\sobnor, 1-d(u',\ephi)\}$, 
using triangle inequality we get 
$
d \left(u'+s v', \ephi \right)<1$ and thus $u'+sv' \in \Pi(\ephi,1)$. 
...
Let $\lambda>0$ such that $\Phi(\frac{v'}{\lambda})\in L^1$.
On the other hand, if $v'\in\lphi$ and $|s|\leq s_0 \lambda^{-1}$,
from the convexity and the parity of $\Phi$, we get
\[
\begin{split}
&\Phi(u'+sv')=
\Phi\left((1-s_0)\frac{u'}{1-s_0}+s_0 \frac{s}{s_0}v'\right)
\leq
(1-s_0)\Phi\left(\frac{u'}{1-s_0}\right)+s_0 \Phi\left(\frac{s}{s_0}v'\right)
\\
&\leq
(1-s_0)\Phi\left(\frac{u'}{1-s_0}\right)+s_0 \Phi\left(\frac{v'}{\lambda}\right)
\in L^1
\end{split}
\]
As $u'\in\Pi(\ephi,1)$ then
\[
d\left(\frac{u'}{1-s_0},E^{\Phi}\right)=\frac{1}{1-s_0}d(u', E^{\Phi})<1
\]
...
\begin{split}
|D_s H(s,t)|&=\left| D_{x}\mathcal{L}(t,u+sv,u'+sv')\ccdot v +  
\lambda D_{y}\mathcal{L}(t, u+s v, u'+sv')\ccdot\frac{v'}{\lambda}\right| \\
  & \leq M \left[ b(t)+ \Phi(u'+sv')\right]|v|\\
 &\quad+ \lambda\left[\Psi(D_{y}\mathcal{L}(t,u+sv,u'+sv'))+\Phi\left(\frac{v'}{\lambda}\right) \right]
\\
...
\left\{D_{x}\mathcal{L}(t,u,u')\ccdot v+ D_{y}\mathcal{L}(t,u,u')\ccdot v'\right\} \ dt.
\]
...
\|D_{y}\mathcal{L}\|_{\lpsi} \|v'\orlnor \leq C \|v\sobnor
\]
...
This is a consequence  of the continuity of the mappings $u \mapsto D_{x}\mathcal{L}(t,u,u')$ and $u \mapsto
D_{y}\mathcal{L}(t,u,u')$. Indeed, if $u_n,u\in \domi_{\Lambda}$ with $u_n\to u$ in the norm of $\wphi$ and $v \in
\wphi$, then
\[
\begin{split}
\left\langle  I'(u_{n}),v \right\rangle &= \int_0^T \left\{  D_{x}\mathcal{L}\left(t,u_n,u'_n\right)\ccdot
v +
 D_{y}\mathcal{L}\left(t,u_n,u'_n\right)\ccdot v'\right\} \ dt\\
&\rightarrow \int_0^T \left\{ D_{x}\mathcal{L}\left(t,u,u'\right)\ccdot v+ 
D_{y}\mathcal{L}\left(t,u,u'\right)\ccdot v'\right\} \ dt\\
&=\left\langle  I'(u),v \right\rangle.
\end{split}
\]


In order to prove item  \ref{it:T1item4}, it is necessary to see that the maps $u\mapsto D_{x}\mathcal{L}(t,u,u')$  
and $u\mapsto D_{y}\mathcal{L}(t,u,u')$  are norm continuous
...
Then, by \eqref{eq:Dy_lambda-Psi} we have
 $ \Psi(v_n) 
	\leq m(t) \in L^1$
 being  $v_n:=D_{y}\mathcal{L}(\cdot,u_n,u'_n)$ and $m(t):= M (b+h)$. 
In addition, from the continuous differentiability of $\mathcal{L}$, we have that
$v_n \to v$ a.e. where $D_y\mathcal{L}(\cdot,u,u')$.
