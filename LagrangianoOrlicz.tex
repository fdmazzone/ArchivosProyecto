...
This paper is concerned with the existence of periodic solutions for the following problem

\begin{equation}\label{ecualagran2}
    \left\{%
\begin{array}{ll}
   \frac{d}{dt} D_{\b{y}}\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))= D_{\b{x}}\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t)) \quad \hbox{a.e.}\ t \in (0,T)\\
    \b{u}(0)-\b{u}(T)=\b{\dot{u}}(0)-\b{\dot{u}}(T)=0,
\end{array}%
\right.
\end{equation}
 where $\b{u}:[0,T]\to\rr^d$ is absolutely continuous and  $\mathcal{L}:\rr\times\rr^d\times \rr^d\to\rr$ satisfies
\begin{eqnarray}
|\mathcal{L}(t,\b{x},\b{y})| &\leq a(|\b{x}|)\left(b(t)+ \Phi\left(\frac{|\b{y}|}{\lambda}+f(t) \right)\right),\label{cotaL}\\
|D_{\b{x}}\mathcal{L}(t,\b{x},\b{y})| &\leq a(|\b{x}|)\left(b(t)+ \Phi\left(\frac{|\b{y}|}{\lambda}+f(t) \right)\right),\label{cotaDxL}\\
|D_{\b{y}}\mathcal{L}(t,\b{x},\b{y})| &\leq a(|\b{x}|)\left(c(t)+ \varphi\left(\frac{|\b{y}|}{\lambda}+f(t)\right)  \right),\label{cotaDyL}
\end{eqnarray}
with  $\lambda>0$,  $a\in C(\mathbb{R}^+, \mathbb{R}^+)$, $\Phi$ is $N$-function, $\varphi$ is the right continuous derivative of $\Phi$,  the functions $b,c,f:[0,T]\to\rr$ satisfies  $b$ integrable,   $c$ and $f$  belonging to  certain  Orlicz spaces associated to $\Phi$.
