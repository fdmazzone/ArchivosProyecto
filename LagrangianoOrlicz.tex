...
Variational problems and hamiltonian systems  have been studied extensively. Classical references of this subject are
\cite{mawhin2010critical,struwe2008variational,ekeland1999convex}. Problems likes \eqref{ProbPrin} maintained the interest of researchers as testifies the recent literature on the topic. For Lagrangians of the type $\mathcal{L}(t,\b{x},\b{y})=\frac{|\b{y}|^2}{2}+F(t,\b{x})$  many solvability conditions were given.  In  \cite{tang1995periodic} the function $F$
...
%which is subquadratic in Rabinowitz{}'s sense 
...
Cristia-2008, 
 GHMS, GHMZ, manasevich2000boundary, manasevich1998periodic} 
...
and where $\b f$ is not a gradient. Our methods differ from previous articles in that  we use the direct method of the calculus of variations.

  In \cite{m-1999} a constrained minimization problem in Orlicz-Sobolev spaces  was considered in order to obtain existence of eigenvalues for certain differential operators involving $N$-functions.  



Finally we mention that the regularity  problem, in an Orlicz-Sobolev stetting, of minimizers of action integrals defined on several variable functions was studied by A. Cianchi  (see \cite{cianchi2000local, cianchi1999gradient}).





In this article  we consider Lagrange functions  defined on Sobolev-Orlicz spaces $W^{1}L^{\Phi}$    
(see \cite{adams_sobolev,KR, rao1991theory,2002applications}) 
and we tackle  the direct method of calculus of variations. The exposition is organized as follows. In Section \ref{preliminares} we present results related to Orliz spaces and Orlicz-Sobolev spaces and results related to composition operators. Almost all result in this section  is essentially known. Conditions \eqref{cotaL}, \eqref{cotaDxL} and \eqref{cotaDyL} are the means to ensure that $I$ is finitely defined on 
a non trivial subset of $W^{1}\lphi_d$ and $I$ is G\^ateaux differentiable in this subspace. We develop this questions in Theorem \ref{teorema_acotacion}  of Section \ref{sec:dif}. In Section \ref{sec:equa-min} we prove that critical point of \eqref{integral_accion} are solution of \eqref{ProbPrin}. Conditions for the coercitivity of action integrals are discussed in Section \ref{sec:coer}. Finally, our main Theorem about existence of solutions of  \eqref{ProbPrin} is introduced in Section \ref{sec:main}. 
