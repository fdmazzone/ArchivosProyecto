...
\end{equation}
where $\alpha_0,\Lambda>0$ and  the function $F:\rr\times\rr^n\to\rr$ is measurable respect to $t$ for every fix  $\b{x}\in\rr^n$ and continuous in $\b{x}$ for a.e. $t\in [0,T]$.
...
\end{equation}
for $C,\nu>0$. If $\Phi(x)=|x|^p/p$ then $J_{C,\nu}$ is clearly coercitive for $\nu<p$. For more general $\Phi$ the situation is more interesting   as will be shown by the following lemma.
...
Let  $\mathcal{L}$ be a Lagrangian function satisfying \eqref{cotaL}, \eqref{cotaDxL}, \eqref{cotaDyL} and \eqref{cota_inf}. We assume  following conditions 
\begin{enumerate}
...
\begin{equation}\label{cotaC}C:=a_0^{-1}T\Psi^{-1}\left(\frac{1}{T}\right)\|b_1\|_{L^1}<1.
\end{equation}
\end{enumerate}
