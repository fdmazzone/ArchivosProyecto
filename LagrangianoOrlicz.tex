...
\begin{comentario}  As a direct consequence of convexity, we can  bound the term $\Phi(|\b{y}|/\lambda+d(t))$, $d\in \ephi_1$, by the expression  $\Phi(|\b{y}|/\Lambda)+b(t)$ where $b(t):=\Phi(2d(t))\in L^1_1$, and  $\Lambda= \lambda/2$. That is, we can  assume $d = 0$ at the price of making smaller the value of $\lambda$. 
\end{comentario}
...
\b{v} +
 D_{\b{y}}\mathcal{L}\left(t,\b{u}_n,\b{\dot{u}}_n\right)\ccdot\dot{\b{v}}\right\} \ dt\\
&\rightarrow \int_0^T \left\{ D_{\b{x}}\mathcal{L}\left(t,\b{u},\b{\dot{u}}\right)\ccdot \b{v}+ 
D_{\b{y}}\mathcal{L}\left(t,\b{u},\b{\dot{u}}\right)\ccdot\dot{\b{v}}\right\} \ dt\\
...
\mathcal{L}(t,\b{x},\b{y})\geq \alpha_0\Phi\left(\frac{|\b{y}|}{\Lambda}-c_0(t)\right)-b_0(t)a_0(|\b{x}|),
\end{equation}
where $\alpha_0,\Lambda>0$, $b_0\in L^1_1([0,T])$ and $a_0\in C(\rr^+,\rr^+)$.  
...
\begin{equation}\label{func_phi}
  J(\b{u}):=\int_0^T\Phi\left(\frac{|\b{u}|}{\Lambda}\right)dt-C\|\b{u}\orlnor,
\end{equation}
for any $C>0$. If $\Phi(x)=|x|^p/p$ then $J$ is clearly coercitive. For more general $\Phi$ the situation is more interesting   as will be shown by the following lemma.

\begin{thm} Let $\Phi$ and $\Psi$ complementary $N$-functions. Then
\begin{enumerate}
  \item If $\Psi \in \Delta_2$ if and only if 
...
\mathcal{L}(t,\overline{\b{u}},\dot{\b{u}}(t))\ dt\\
\geq & -C_1\|f\|_{L^1}\|\dot{\b{u}}\|_{\lphi} + \int_{0}^T \mathcal{L}(t,\overline{\b{u}},\dot{\b{u}}(t))\ dt.
\end{split}
\]
...
En la última secci\'on dej\'e los intentos previos en caso de que alguno deba ser rescatado del ostracismo.}


\section{Algo para rescatar????}
\begin{nota_fer}
\textbf{Tenemos la conjetura que todo sale si pedimos $\Phi$ que satisface $\nabla_2$. Problema: probarla o refutarla} Un problema más amplio sería tratar de sacar hipótesis ya sea la condición $\Delta_2$ de alguna de las funciones ($\Phi$ o $\Psi$) o usar estas condiciones para el infinito o aún para cero(????).
\end{nota_fer}
...
\begin{nota_fer}  Si me parece que habría que decir que entendemos por coercitivo en el teorema de abajo. Yo creo que la demostración \textbf{Problema: verificarlo y escribarlo} implica el sentido más fuerte de coercitividad
...
\begin{nota_fer}
Lamentablemente el libro de Rao pag. 43 dice que $\nabla'$ implica $\nabla_2$. Luego la primera parte de este terorema es consecuencia de la proposición anterior.
 El otro inciso si me parece que tiene sentido pues para ver que no es coercitiva basta mostrar contraejemplos. Este comentario no aporta problemas abiertos, solo es una observación de la redacción
\end{nota_fer} 
...
\begin{nota_fer} Me parece que es posible ver que no es coercitiva en el sentido más debil que
\[\int \Phi(|u_n|)\,dx-C\|u_n{\orlnor}\]
no tiende a infinito.\textbf{Demostrarlo}
Por otra parte, notar que al final de la demostración se toma $\alpha=r$, como lo que estamos mostrando es un contraejemplo, no es necesario mantener la distinción entre $\alpha$ y $r$. Podríamos haber tomado de movida 
\[u=u_r=r\chi_{[0,1/r]},\]
\end{nota_fer}
...
\begin{nota_fer} Deje de entender. Al fin y al cabo, el de abajo sería un contraejemplo para mostrar la misma cosa que ya vimos en otro contraejemplo? 


Pero se nos plantea este problema, la $\Phi$ del contraejemlo de abajo no será $\nabla_3$? Si fuese así lo que decimos abajo ya lo sabemos \textbf{Problema abierto: probar o refutar que la $\Phi$ es $\nabla_3$}
\end{nota_fer}
...
{\color{green}  Hay una diferencia entre el enunciado general y el ejemplo de abajo. El ejemplo de abajo es un contraejemplo a la afirmación
...
\end{equation}
Como \eqref{eq:paraborrar2} implica \eqref{eq:paraborrar},  el contraejemplo de abajo no es consecuencia del resultado general. Además nosotros lo que necesitamos es \eqref{eq:paraborrar2}. Todo esto nos lleva a:
