...
\end{equation}
where $\alpha_0,\Lambda>0$ and  $F:\rr\times\rr^n\to\rr$ is a Caratheodory function, i.e. $F(t,\b{x})$ is  measurable respect to $t$ for every fixed  $\b{x}\in\rr^n$ and it is continuous in $\b{x}$ for a.e. $t\in [0,T]$. We note that in virtue of \eqref{cota_inf} and \eqref{cotaL} we have that $F(t,\b{x})\leq a(|\b{x}|)b_0(t)$, with $b_0(t):=b(t)+\Phi(d(t))\in L^1_1([0,T])$. In order to ensure that integral $\int_0^TF(t,\b{u})dt$ be finite for $\b{u}\in\wphi$,  we need to assume 
