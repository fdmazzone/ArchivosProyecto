...
\[
|\langle \b{v},I'(\b{u}) \rangle| \leq c \|v\linf + c \|\b{\dot{v}}\orlnor \leq c \|\b{v}\sobnor.
\]
...
\[
\begin{split}
\left\langle \b{v}, I'(\b{u}_{n}) \right\rangle &= \int_0^T \left\{  D_{\b{x}}\mathcal{L}\left(t,\b{u}_n,\b{\dot{u}}_n\right)\ccdot
\b{v} +
...
from $\domi $ into $L^1$ and
 $\lpsi$ respectively.  The continuity of the first map has already been proved in step 1. We will prove the continuity of the second map.  We consider $\b{u}_n$ and $\b{u}$ in $\domi$ with $\|\b{u}_n- \b{u}\sobnor\to 0$.
By Lemma \ref{segundo lema}, there exist a subsequence $\b{u}_{n_k}$ and $h\in \domi$ such that $\b{u}_{n_k}\rightarrow \b{u} \quad\text{a.e.}$, $\b{\dot{u}}_{n_k}\rightarrow \b{\dot{u}} \quad\text{a.e.}$ and $|\b{\dot{u}}_{n_k}|\leq h\quad\text{a.e.}$.


 Then  since $\mathcal{L}$ is a Caratheodory function
 we have $ D_{\b{y}}\mathcal{L}(t,\b{u}_{n_k}(t),\b{\dot{u}}_{n_k}(t))\to D_{\b{y}}\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))$ a.e. $t\in [0,T]$. By an argument similar to a previous one, and  using \eqref{cotaDyL} that $\Psi$ is of the $\Delta_2$ class, we obtain
 \[\begin{split}
