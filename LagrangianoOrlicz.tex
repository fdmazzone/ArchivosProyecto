...
\end{enumerate}
Moreover if $L(t,x,y)$ is $T$-periodic with repect to the variable $t$ and strictly convex with respect to $y$, then $D_yL(0,u(0),\dot{u}(0))-D_yL(T,u(T),\dot{u}(T))=0$ is 
...
Using \cite[pag. 6]{mawhin2010critical} we obtain that $D_yL(t,u(t),\dot{u}(t))$ is absolutely continuous, therefore it is differentiable a.e.on $[0,T]$ and that the first equality of \ref{ecualagran} holds true. Moreover the same reference says that $D_yL(t,u(t),\dot{u}(t))$ is $T$-periodic. This complete the proof of 1. implies 2. The converse is still easier than the part we have just done. 2. implies 1. ,.....
