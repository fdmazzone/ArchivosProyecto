...
It is well known that coercitivity is an ingredient useful in order to establish existence of minima. Therefore we are interestent in finding conditions which insure the coercitivityof the action integral $I$ acting on $\domi$. For this purpose we need to introduce the following  extra condition on Lagrange function $\mathcal{L}$  
\begin{equation}\label{cota_inf}
\mathcal{L}(t,\b{x},\b{y})\geq \alpha_0\Phi\left(\frac{|\b{y}|}{\Lambda}\right)+F(t,\b{x}),
\end{equation}
where $\alpha_0,\Lambda>0$ and $F:\rr\times\rr^n\to\rr$ is a Caratheory function.  



As we shall see in Theorem \ref{coercitividad1}, when $\mathcal{L}$ satisfies \eqref{cotaL}, \eqref{cotaDxL}, \eqref{cotaDyL} and \eqref{cota_inf},  the coercitivity of the action integral $I$ is related to the coercitivity of the functional
\begin{equation}\label{func_phi}
  J_C(\b{u}):=\int_0^T\Phi\left(\frac{|\b{u}|}{\Lambda}\right)dt-C\|\b{u}\orlnor,
\end{equation}
for $C>0$. If $\Phi(x)=|x|^p/p$ then $J_C$ is clearly coercitive. For more general $\Phi$ the situation is more interesting   as will be shown by the following lemma.

\begin{lem} Let $\Phi$ and $\Psi$ complementary $N$-functions. Then
\begin{enumerate}
  \item If $C\Lambda<1$ then $J_C$ is coercitive. 
  
  \item If $\Psi$ is $\Delta_2$ global, then there exist $\mu>1$ such that
...
\end{equation}
In particular, the functional $J_C$ is coercitive for every $C>0$.
\item If $J_C$ is coercitive with $C\Lambda>1$, then $\Psi$ is $\Delta_2$ at infinity.  
\end{enumerate}
\end{lem}
...
Then
\[\frac{(1-C\Lambda)}{\Lambda}\|\b{u}\orlnor-1\leq \rho_{\Phi}\left(\frac{\b{u}}{\Lambda}\right)- C\|\b{u}\orlnor=J_C(\b{u}).\]
This show that $J_C$ is coercitive, and therefore it proves item 1.  

In virtue of \cite[Eq. (2.8)]{AGMS}, the $\Delta_2$ condition for $\Psi$, \cite[Th 11.7]{M} and \cite[Cor. 11.6]{M} we obtain a constant $K>0$ and $\nu>1$ such that for any $s\geq 0$ and $\lambda>1$ 
\begin{equation}\label{delta2-consecuencia}
\Phi(r s)\geq Kr^{\nu}\Phi(s).
\end{equation}
Let $1<\mu<\nu$ and consider $r>\Lambda$ that later will be specify.  Then, from \eqref{delta2-consecuencia} and \eqref{amemiya}, we get
\[
\begin{split}
...
\[
\begin{split}
J_C(\b{u}_n)&=\int_0^T \Phi\left(\frac{|\b{u}_n|}{\Lambda}\right)-C\|\b{u}_n\orlnor\\
&=
...
\end{split}
\]
From \eqref{eq: un-_tiende_inf} and the condition $C\Lambda>1$, we obtain  $J_C(\b{u}_n)\to+\infty$, which is a contradiction.
\end{proof}
...
Let  $\mathcal{L}$ be a Lagrangian function satisfying \eqref{cotaL}, \eqref{cotaDxL}, \eqref{cotaDyL} and \eqref{cota_inf}. We assume that the function $F$ in \eqref{cota_inf} satisfies the following conditions 
\begin{enumerate}
\item There exists $f \in L^1_1$  such that
\[
\left|D_{\b{x}} F(t,\b{x}) \right|\leq f(t)
\]
for a.e. $t \in [0,T]$ and all $\b{x}\in \mathbb{R}^n $. 
\item 
...
\begin{enumerate}
\item[3.]  $\Psi$ is $\Delta_2$ or 
\begin{equation}\label{cotaC}C:=a_0^{-1}T\Psi^{-1}\left(\frac{1}{T}\right)\|f\|_{L^1}<1.
\end{equation}
Recall that the constant $a_0$ has been given in  \eqref{cota_inf}. 
\end{enumerate}
Then  the action integral $I$ is coercive.
\end{thm}

\begin{proof} In the following estimates we will use the descomposition $\b{u}=\b{\overline{u}}+\b{\tilde{u}}$ and the Wirtinger inequality \eqref{wirtinger}. 
\[
\begin{split}
...
&=a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)+ \int_0^T\int_0^1D_{\b{x}}F(t,\b{\overline{u}}+s\b{\tilde{u}})\ccdot\b{\tilde{u}}dsdt +  \int_0^TF(t,\b{\overline{u}})dt\\
&\geq a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right) -T\Psi^{-1}\left(\frac{1}{T}\right)\|f\|_{L^1}\|\b{\dot{u}}\|_{\lphi} + \int_0^TF(t,\b{\overline{u}})dt\\
&=a_0J_C(\b{\dot{u}})+ \int_0^TF(t,\b{\overline{u}})dt,
\end{split}
\]
where $C$ is the constant defined in \eqref{cotaC}.
