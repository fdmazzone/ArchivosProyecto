...
Given a function $\varphi$ as above, we also consider the so-called right inverse function $\psi$ of $\varphi$ which is 
defined by $\psi(s)=\sup_{\varphi(t)\leq s}t$.
The function $\psi$ satisfies the same properties that function $\varphi$, therefore we have an $N$-function $\Psi$ such that $\Psi'=\psi$ .
...
\]
is a Banach space. By $\b{u}\b{\cdot} \b{v}$ we denote the usual dot product in $\mathbb{R}^{d}$ between $\b{u}$ and $\b{v}$.  Sometimes the following alternative expression for the norm, known as \emph{Amemiya norm},     will  be useful (see \cite[Thm. 10.5]{KR} and \cite{hudzik2000amemiya}). For every $\b{u}\in\lphi$,
...
More concretely,  there exists a non decreasing function $c:\mathbb{R}^+\to\mathbb{R}^+$ such that
 $\|\b{a}(\b{u})\|_{L^{\infty}([0,T])}\leq c(\|\b{u}\|_{\wphi})$.
{\color{red} La letra c es confusa!!! Cambiar por $A$}
\end{cor}

\begin{proof}  Let $\alpha\in C(\mathbb{R}^+,\mathbb{R}^+)$ be a  non-decreasing majorant of $a$, for example $\alpha(s):=\sup_{0\leq t\leq s}a(t)$.  If $\b{u}\in \wphi_d$ then, by Lemma \ref{inclusion orlicz}, 
\[a(|\b{u}(t)|)\leq \alpha(\|\b{u}\|_{L^{\infty}})\leq a\left(\Phi^{-1}\left(\frac{1}{T}\right)\max\{1,T\} \|\b{u}\|_{\wphi}\right)=: c(\|\b{u}\|_{\wphi}).\]
\end{proof}
...
%where, as consequence of Lemma \ref{phi_comp} and the $\Delta_2$ condition for $\Phi$, we have   $c_1(t):=c(t)+K\varphi\left(d(t)\right)\in \lpsi_1$. 
%
...
\begin{comentario} If $\Phi\in\Delta_2$ then we can asssume $d=0$ keeping the same value of $\lambda$. This is consequence of that a non decreasing $\Delta_2$ function $G:\mathbb{R}^+\to\mathbb{R}^+$ is quasi-subadditive. In fact, we suppose $s_1\leq s_2$, then  
...
Thus, as $d\in\ephi_1$ and attending to \eqref{inclusiones}, we get 

\begin{equation}\label{inclusion3}
|\b{\dot{u}}|/\lambda+d\in\Pi(\ephi_1,1)\subset \claseor_1.
\end{equation}
By Corollary \ref{a_bound} we get a constant $c=c(\|\b{u}\sobnor )$ such that  $a(|\b{u}(t)|)\leq c$, $t\in [0,T]$.
 Thus,
...
\[
  \left | D_y\mathcal{L}(\cdot,\b{u}_n,\b{\dot{u}}_n) \ccdot \b{ v} \right| 
	\leq C\left(c+\varphi\left(\frac{h}{\lambda}+d\right)\right)|\b{v}|\in L_1^1.
\]
...
\end{split}
\end{equation}
 Invoking \eqref{DxL1}, \eqref{DyLpsi} with $ |\b{\dot{u}}|+s_0|\b{\dot{v}}|$ instead $\b{\dot{u}}$ and taking account of $\b{v}\in L^{\infty}_d$ and $\b{\dot{v}}\in\lphi_d$, we show that there exists a function $g \in L^1_1([0,T], \mathbb{R}^{+})$
...
\]
{\color{red}{decir en ingl\'es que $c$ es cualquier constante Si!!!.}}
\\
...
By Lemma \ref{segundo lema}, there exist a subsequence $\b{u}_{n_k}\in \domi$ and a function $h\in\Pi(\ephi_1,\lambda)$ such that $\b{u}_{n_k}\rightarrow \b{u} \quad\text{a.e.}$, $\b{\dot{u}}_{n_k}\rightarrow \b{\dot{u}} \quad\text{a.e.}$ and $|\b{\dot{u}}_{n_k}|\leq h\quad\text{a.e.}$ Then,  since $\mathcal{L}$ is a Carath\'eodory function
 ...
 \[\begin{split}
    ...
    &\leq C\left( c(t) + \varphi \left(\frac{|h(t)|}{\lambda}+d(t)\right)\right)\in \lpsi_1=\epsi_1.
   \end{split}
\]
...
In this section we derive the Euler-Lagrange equations associated to critical points of action integrals.  We denote by $\wphi_T$ the subspace of $\wphi_d$ containing all  $T$-periodic functions. Similarly we consider the subspaces $\ephi_T$, $\lphi_T$. As usual, when $Y$ is a subspace of
...
We recall that  a function $f: \mathbb{R}^d \to \mathbb{R}$ is called \emph{strictly convex} if $f\left(\tfrac{\b{x}+\b{y}}{2}\right)< \tfrac{1}{2} \left(f\left(
...
Using \cite[pp. 6--7]{mawhin2010critical} we obtain that  $D_{\b{y}}\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))$ is absolutely continuous and $T$-periodic, therefore it is differentiable a.e. on $[0,T]$ and the first equality of \eqref{ecualagran2} holds true.
...
The last part of the theorem is a consequence of that $D_{\b{y}}\mathcal{L}(T,\b{u}(T),\b{\dot{u}}(T))=D_{\b{y}}\mathcal{L}(0,\b{u}(0),\b{\dot{u}}(0))=D_{\b{y}}\mathcal{L}(T,u(T),\b{\dot{u}}(0))$ and the injectivity of $D_{\b{y}}\mathcal{L}(T,u(T),\cdot)$.
\end{proof}
...
\end{equation}
where $\alpha_0,\Lambda>0$ and  $F:\rr\times\rr^d\to\rr$ is a Caratheodory function, i.e. $F(t,\b{x})$ is  measurable respect to $t$ for every fixed  $\b{x}\in\rr^n$ and it is continuous in $\b{x}$ for a.e. $t\in [0,T]$. We note that in virtue of \eqref{cota_inf} and \eqref{cotaL} we have that $F(t,\b{x})\leq a(|\b{x}|)b_0(t)$, with $b_0(t):=b(t)+\Phi(d(t))\in L^1_1([0,T])$. In order to ensure that integral $\int_0^TF(t,\b{u})dt$ be finite for $\b{u}\in\wphi$,  we need to assume 
...
In virtue of \cite[Eq. (2.8)]{AGMS}, the $\Delta_2$-condition for $\Psi$, \cite[Thm. 11.7]{M} and \cite[Cor. 11.6]{M} we obtain a constant $K>0$ and $\alpha_{\Phi}>1$ such that for any $0<\nu<\alpha_{\Phi}$, $s\geq 0$ and $r>1$ 
...
Following \cite{mawhin2010critical} we said that $F$ satisfies the condition (A) if  $F(t,\b{x})$ is a Caratheodory function, $F$ satisfies \eqref{condA1} and $F$ is continuously differentiable with respect to $\b{x}$. Moreover, the following inequality holds 
