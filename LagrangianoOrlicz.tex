...
\right.
\end{equation}
where $T>0$ and $\mathcal{L}:[0,T]\times\rr^n\times\rr^n\to\rr$ is a Caratheodory function satisfying the conditions 
\begin{eqnarray}
...
exists $\lambda>0$ and non negative functions   $a \in C(\mathbb{R}^+, \mathbb{R}^+)$, $b \in L^1_1([0,T])$, $c \in \lpsi_1([0,T])$ and $f\in\ephi_1$ such that
...
In this section we give conditions for the existence  of critical points of $I$. More precisely we give suficient conditions for that the following optimization problem has solutions 
\begin{equation}\label{minimi_prob}\min\limits_{\b{u}\in\wphi_T}I(\b{u})=\min\limits_{\b{u}\in\wphi_T}\int_0^T\mathcal{L}(t,\b{u},\b{\dot{u}})dt.
\end{equation}
...
\[I(\b{u})\leq \liminf_{n\to\infty}I(\b{u}_n).\]
...
Let $\{\b{u}_n\}$ be a sequence  satisfying the assumptions of the Theorem.   We define the Caratheodory function $\mathcal{L}'=\mathcal{L}-F$. 
...
Let $\{\b{u}_n\}\subset \wphi_T$ be a minimizing sequence of the problem \cite{minimi_prob}. Then $I(\b{u}_n)$ is bounded. The coercitivity conditions imply that $\{\b{u}_n\}$ is norm bounded in $\wphi_d$. Therefore 
