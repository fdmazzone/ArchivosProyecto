...
Likes in \cite{KR}, we utilize the subset $\Pi(\ephi,r)=\Pi\left(\ephi\left([0,T],\mathbb{R}^n\right),r\right)$ of $\lphi([0,T],\mtahbb{R}^n)$ defined by
...
\begin{proof}
Let $r:=d(u,\ephi)$, $r<1$. Because $u_n$ converges to $u$, for
each $k\in\mathbb{N}$ there exists $n_k$ such that
\[\|u_{n_k}-u\|_{\Phi}<\frac{1-r}{2}\quad \text{ and }\quad \|u_{n_k}-u_{n_{k+1}}\|_{\Phi}<2^{-k-1}(1-r)\]
Let
$S_k=u_{n_1}+(u_{n_2}-u_{n_1})+\ldots+(u_{n_k}-u_{n_{k-1}})$, and let
$h:[0,T]\rightarrow\mathbb{R}$ defined by
\[ h(x)=|u_{n_1}(x)|+\sum_{k=2}^{\infty}|u_{n_k}(x)-u_{n_{k-1}}(x)|.\]
From $d(u_{n_1},\ephi)\leq d(u_{n_1},u)+d(u,\ephi)<\frac{1+r}{2}$, we can see that
$h\in\Pi(\ephi,1)$. Because
$\Pi(\ephi,1)\subset \claseor$, we know that $\int_{[0,T]}
...
\begin{lem}
We suppose that $L(t,x,y)$ is a Charateodory functions satisfying \eqref{cotaL}-\eqref{cotaDyL}. Moreover we assume $L(t,x,\cdot)$ is convex for each $t,x$. We suppose that $\Phi,\Psi\in\Delta_2$.  
...
\[
\begin{split}I(v)&=\int_0^TL(t,v(t),\dot{v}(t))dt\\
 &=\int_0^TL(t,v(t),\dot{v}(t))-L(t,u(t),\dot{v}(t))dt +\int_0^TL(t,u(t),\dot{v}(t))dt\\
 &=:J(v)+H(v).
\end{split}
\]
 ...
 \begin{split}
   &\left|  L(t,u_n(t),\dot{u}_n(t))-L(t,u(t),\dot{u}_n(t))\right|\\
  &\leq|D_xL(t,\xi_n(t),\dot{u}_n(t))||u_n(t)-u(t)|.
  \end{split}
 \end{equation}
...
\begin{equation}\label{acot_Dx}
 \left|D_xL(t,\xi_n(t),\dot{u}_n(t))\right|\leq C \left(b(t)+\Phi(|\dot{u}_n(t))|)\right)
\end{equation}
...
such that
\[|L(t,u(t),\dot{u}_n(t))-L(t,u(t),\dot{u}(t))|\leq|D_yL(t,u(t),(1-s)\dot{u}_n(t)+s \dot{u}(t))||\dot{u}_n-\dot{u}|.\]
...
\[\begin{split}|L(t,u(t),\dot{u}_n(t))-L(t,u(t),\dot{u}(t))|&\leq C\left(c(t)+\varphi(\chi_{\mathfrak{G}}|\dot{u}_n(t)|+
 ...
 Which completes the proof (o eso me gustaría creer a mi)
