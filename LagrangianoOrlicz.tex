...
The subspace $\ephi_n=\ephi_n([0,T])$ is defined as the closure in $\lphi_n$ of the subspace $L^{\infty}_n$ of all the $\mathbb{R}^n$-valued essentially bounded functions. It is showed that  $\ephi_n$ is the maximal subspace contained in the Orlicz class $\claseor$. Equivalently $\b{u}\in\ephi_n$ if and only if for any $\lambda>0$ we have $\rho_{\Phi}(\lambda \b{u})<\infty$. 

 


 and, it is known that, $\left[\ephi\right]^*=\lpsi$.
