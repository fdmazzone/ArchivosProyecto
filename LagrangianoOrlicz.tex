...
\end{split}
\end{equation}
As $\Phi$ is $N$-function, $\lim_{\xi\to \infty} \Phi^{-1}(\xi)/\xi=\lim_{\eta\to \infty} \eta/\Phi(\eta)=0$.  This fact and \eqref{equicont} prove that the sequence $\{\b{u}_{k}\}_{k \geq 1}$ is equicontinuous. By Arzela-Ascoli theorem, $\b{u}_{k}$ converges uniformly to a function $\b{v}\in C([0,T],\mathbb{R}^n)$. In particular $\b{u}_{k}\rightharpoonup \b{v}$ in $\lphi$. From the immersion $\wphi  $\b{u}_{k}\rightharpoonup \b{u}$ in $\lphi$

 By the uniqueness of the weak limit in $C(0,T;\mathbb{R}^n)$,
