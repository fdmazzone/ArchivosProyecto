...
\begin{align}
  &\hbox{a) }\|\widetilde{\b{u}}\|_{L^{\infty}} \leq T\Phi^{-1}\left(\frac{1}{T}\right)\|\b{\dot u}\orlnor&\text{  (Wirtinger's inequality)}\label{wirtinger}\\
&\hbox{b) } \|\b{u}\|_{L^{\infty}} \leq\Phi^{-1}\left(\frac{1}{T}\right)\max\{1,T\}\|\b{u}\sobnor&\text{  (Sobolev's inequality)}\label{sobolev}\\
&\hbox{c) }  \left|\b{u}(t)-\b{u}(s) \right| \leq  \|\b{\dot{u}}\orlnor ( t-s)\Phi^{-1}\left(\frac{1}{t-s}\right).&
\end{align}
\end{lem}
\begin{comentario}
 As is usual, a function   $w:[0,+\infty)\to[0,+\infty)$ is called  a \emph{modulus of continuity} if $w$ is a continuous increasing function which satisfies $w(0)=0$. We say that $\b{u}:[0,T]\to\rr^d$  has modulus of continuity $w$  when there exists a constant $C>0$ such that 
\[|\b{u}(\b{x})-\b{u}(\b{y})|\leq Cw(|\b{x}-\b{y}|).\] 
Lemma \ref{inclusion orlicz} c) establish that $w(s):=s\Phi^{-1}(1/s)$ is a modulus of continuity for all functions $\b{u}\in\wphi$. We note that since $\Phi$ is a $N$-function we get $w(0)=0$.  
...
Inequality c) is obtained as follows. For $0 \leq
...
\begin{comentario} If $\Phi\in\Delta_2$ then we can asssume $f=0$ keeping the same value of $\lambda$. This is consequence of that a non decreasing $\Delta_2$ function $G:\mathbb{R}^+\to\mathbb{R}^+$ is quasi-subadditive (see \cite[Prop. 4.2]{AF12}) i.e. there exists $K>0$  
