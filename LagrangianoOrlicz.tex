...
\[
  \left |\langle \left|D_y\mathcal{L}(t,\b{u}_n(t),\b{\dot{u}}_n(t)) ,\b{ v}\rangle \right| \leq C\left(c(t)|\b{v}(t)|+\varphi(|h(t)|)|\b{v}(t)|\right).
\]
We have that $c(t),\varphi(h(t))\in\lpsi$ and $|\b{v}|\in\lphi$. Then, the H\"older inequality for Orlicz spaces implies that  $D_y\mathcal{L}(t,\b{u}_{n_k}(t),\dot{\b{u}}_{n_k}(t))\cdot v$ is dominated by a function in $L^1$. Thus, from the Lebesgue dominated convergence Theorem we deduce
...
Finally we prove \ref{T1item3}. The proof follows the similar lines that \cite[Theorem 1.4]{mawhin2010critical}. For $\b{u}\in \domi$ and $v\in\wphi$ we define the function
\[f(s,t)=\mathcal{L}(t,\b{u}(t)+sv(t),\dot{\b{u}}(t)+s\dot{v}(t)).\]
We remark that by \ref{T1item1}
\[I(\b{u}+sv)=\int_0^Tf(s,t)\ dt\]
is well defined and it is finite valued for $t\in [0,T]$ and  $|s|\leq s_0:=\left(1-d(\dot{\b{u}},\ephi)\right)/\|v\sobnor$ ($v\neq 0$).

Applying the chain rule and the inequalities \eqref{cotaDxL}-\eqref{cotaDyL} we obtain
\begin{equation}\label{ctg}
\begin{split}
|D_s f(s,t)|&=|D_x\mathcal{L}(t,\b{u}+sv,\dot{\b{u}}+s\dot{v})\cdot v + D_y\mathcal{L}(t,\b{u}+sv,\dot{\b{u}}+s\dot{v})\cdot \dot{v}| \\
&\leq a(|\b{u}+sv|)\left[(b(t)+\Phi(|\dot{\b{u}}+s\dot{v}|))|v|+ (c(t)+ \varphi(|\dot{\b{u}}+s\dot{v}|))|\dot{v}|\right]\\
& \leq a_0 \left[(b(t)+\Phi(|\dot{\b{u}}|+s_0|\dot{v}|))|v|+ (c(t)+ \varphi(|\dot{\b{u}}|+s_0|\dot{v}|))|\dot{v}|  \right]
\end{split}
\end{equation}
where
\[
...
\[
d \left(|\dot{\b{u}}|+s_0|\dot{v}|, \ephi \right) %d(|\dot{\b{u}}|,\ephi)+ d(|\dot{\b{u}}|+s_0|\dot{v}|, |\dot{\b{u}}|)
\leq d \left(|\dot{\b{u}}|,\ephi \right)+ s_0 \|\dot{v}\orlnor < 1.
\]
As a consequence $|\dot{\b{u}}|+s_0|\dot{v}| \in \Pi(\ephi,1) \subset \claseor$. Then $b+\Phi(|\dot{\b{u}}|+s_0|\dot{v}|) \in L^1$ and since $v \in L^{\infty}$ we have that
$(b+\Phi(|\dot{\b{u}}|+s_0|\dot{v}|))|v| \in L^1$. On the other hand, we recall that $\boldsymbol{\varphi}$ acts from  $\Pi(\ephi,1)$ into $\tilde{L}^{\Psi}$ therefore
$c(t)+ \varphi(|\dot{\b{u}}|+s_0|\dot{v}|) \in L^{\Psi}$ and since $\dot{v} \in L^{\Phi}$, applying the H\"older inequality
$(c(t)+ \varphi(|\dot{\b{u}}|+s_0|\dot{v}|))|\dot{v}| \in L^1$. Thus, from \eqref{ctg} and the above discussion there exists a function $g \in L^1([0,T], \mathbb{R}^{+})$
such that $|D_s f(s,t)| \leq g(t)$. Consequently, $I$ has a directional derivative and
\[
\langle I'(\b{u}), v \rangle=\frac{d}{ds}I(\b{u}+sv)\big|_{s=0}=\int_0^T \left(D_{x}\mathcal{L}(t,\b{u},\dot{\b{u}})\cdot v+ D_{y}\mathcal{L}(t,\b{u},\dot{\b{u}})\cdot\dot{v}\right) \ dt.
\]
Moreover, from \eqref{DxL1}, \eqref{DyLpsi}, Lemma \ref{inclusion orlicz} and previous formula
\[
|\langle I'(\b{u}), v \rangle| \leq c \|v\linf + c \|\dot{v}\orlnor \leq c \|v\sobnor.
\]
...
\begin{proof}
Let us see that the maps $u\mapsto D_x\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))$  and $u\mapsto D_y\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))$  are continuous
from $\left(\wphi, \|\cdot \sobnor\right) $ into $\left( L^1, \|\cdot \|_{L^1}\right)$ and
 ...
 we have $ D_y\mathcal{L}(t,u_{n_k}(t),\dot{u}_{n_k}(t))\to D_y\mathcal{L}(t,u(t),\dot{u}(t))$ a.e. $t\in [0,T]$.  From Lemma \ref{inclusion orlicz} and the fact that $\|u_{n_k}\sobnor$ are uniformly bounded, we get that there exists $N>0$ such that
 ...
 \[\begin{split}
    |D_y\mathcal{L}(t,u_{n_k}(t),\dot{u}_{n_k}(t))| &\leq a(|u_{n_k}|)\left( c(t) + \varphi (|\dot{u}_{n_k}(t)|)\right)\\
...
extract a subsequence with $\|D_y\mathcal{L}(t,u_{n_k},\dot{u}_{n_k})-D_y\mathcal{L}(t,u,\dot{u})\|_{\lpsi}\to 0$. The desired result follows from a standard argument.

\[
  \begin{split}
    \|I'(u)-I'(u_0)\|_{\left[\wphi\right]^*}&=\sup_{v\in \wphi,\|v\sobnor\leq 1} \langle I'(u)-I'(u_0),v \rangle\\
      &=\sup_{v\in \wphi, \|v\sobnor\leq 1} \int_0^T \left\{ \left(D_x\mathcal{L}(t,u(t),\dot{u}(t))-D_x\mathcal{L}(t,u_0(t),\dot{u_0}(t))\right)\cdot v(t)\right.\\
      &\quad\left.+
      \left(D_y\mathcal{L}(t,u(t),\dot{u}(t))-D_y\mathcal{L}(t,u_0(t),\dot{u_0}(t))\right)\cdot \dot{v}(t)\right\}dt \\
      &\leq \sup_{v\in \wphi, \|v\sobnor\leq 1} \left\{\| D_x\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))-D_x\mathcal{L}(\cdot,u_0(\cdot),\dot{u_0}(\cdot))\|_{L^1}\|v\|_{L^{\infty}}\right.\\
      &\quad+\left.
       \| D_y\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))-D_y\mathcal{L}(\cdot,u_0(\cdot),\dot{u_0}(\cdot))\|_{\lpsi}\|\dot{v}\|_{\lphi}\right\}\\
       &\leq C\left(\| D_x\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))-D_x\mathcal{L}(\cdot,u_0(\cdot),\dot{u_0}(\cdot))\|_{L^1}\right.\\
       &\left. \quad +
       \| D_y\mathcal{L}(\cdot,u(\cdot),\dot{u}(\cdot))-D_y\mathcal{L}(\cdot,u_0(\cdot),\dot{u_0}(\cdot))\|_{\lpsi}\right)
  \end{split}
\]
...
\begin{equation}\label{ecualagran}
    \left\{%
\begin{array}{ll}
   \frac{d}{dt} D_{y}\mathcal{L}(t,u(t),\dot{u}(t))= D_{x}\mathcal{L}(t,u(t),\dot{u}(t)) \quad \hbox{a.e.}\ t \in (0,T)\\
...
\begin{enumerate}
 ...
 \item  $D_y\mathcal{L}(t,u(t),\dot{u}(t))dt$ is an absolutely continuous function and $u$ solve the following boundary value problem
 \begin{equation}\label{ecualagran2}
    \left\{%
\begin{array}{ll}
   \frac{d}{dt} D_{y}\mathcal{L}(t,u(t),\dot{u}(t))= D_{x}\mathcal{L}(t,u(t),\dot{u}(t)) \quad \hbox{a.e.}\ t \in (0,T)\\
    u(0)-u(T)=D_y\mathcal{L}(0,u(0),\dot{u}(0))-D_y\mathcal{L}(T,u(T),\dot{u}(T))=0.
\end{array}%
\right.
\end{equation}
...
$D_y\mathcal{L}(0,u(0),\dot{u}(0))-D_y\mathcal{L}(T,u(T),\dot{u}(T))=0$ is equivalent to $\dot{u}(0)=\dot{u}(T)$.
...
\[\int_0^TD_y\mathcal{L}(t,u(t),\dot{u}(t))\cdot \dot{v}(t)dt=-\int_0^TD_x\mathcal{L}(t,u(t),\dot{u}(t))\cdot v(t)dt \]
Using \cite[pag. 6]{mawhin2010critical} we obtain that $D_y\mathcal{L}(t,u(t),\dot{u}(t))$ is absolutely continuous, therefore it is differentiable a.e.on $[0,T]$ and that the first equality
of \ref{ecualagran} holds true. Moreover the same reference says that $D_y\mathcal{L}(t,u(t),\dot{u}(t))$ is $T$-periodic.
This complete the proof of 1. implies 2. The converse is still easier than the previous one, we omit the proof of it.

The last part of the Corollary is a consequence of that $D_y\mathcal{L}(T,u(T),\dot{u}(T))=D_y\mathcal{L}(0,u(0),\dot{u}(0))=D_y\mathcal{L}(T,u(T),\dot{u}(0))$ and the well known fact strictly
...
\[
\begin{split}I(v)&=\int_0^T\mathcal{L}(t,v(t),\dot{v}(t))dt\\
 &=\int_0^T\mathcal{L}(t,v(t),\dot{v}(t))-\mathcal{L}(t,u(t),\dot{v}(t))dt +\int_0^T\mathcal{L}(t,u(t),\dot{v}(t))dt\\
 &=:J(v)+H(v).
\end{split}
\]
 ...
 \begin{split}
   &\left|  \mathcal{L}(t,u_n(t),\dot{u}_n(t))-\mathcal{L}(t,u(t),\dot{u}_n(t))\right|\\
  &\leq|D_x\mathcal{L}(t,\xi_n(t),\dot{u}_n(t))||u_n(t)-u(t)|.
  \end{split}
 \end{equation}
...
\begin{equation}\label{acot_Dx}
 \left|D_x\mathcal{L}(t,\xi_n(t),\dot{u}_n(t))\right|\leq C \left(b(t)+\Phi(|\dot{u}_n(t))|)\right)
\end{equation}
...
there exists a constant $C>0$ such that $\|\Phi(|\dot{u}_n(t))|)\|_{L^1}\leq C$. Then, from 
...
such that
\[|\mathcal{L}(t,u(t),\dot{u}_n(t))-\mathcal{L}(t,u(t),\dot{u}(t))|\leq|D_y\mathcal{L}(t,u(t),(1-s)\dot{u}_n(t)+s \dot{u}(t))||\dot{u}_n-\dot{u}|.\]
Let $\mathfrak{G}$ be the set $\{|\dot{u}_n(t)|\geq |\dot{u}(t)|\}$. Then
\[|(1-s)\dot{u}_n(t)+ s\dot{u}(t)|\leq \max\{|\dot{u}_n(t)|,|\dot{u}(t)|\}=\chi_{\mathfrak{G}}(t)|\dot{u}_n(t)|+
 \chi_{\mathfrak{G}^c}(t)|\dot{u}(t)|
\]
Therefore, using \eqref{cotaDyL} and by the uniform boundedness of $a(|u_n(t)|)$ we get
\[\begin{split}|\mathcal{L}(t,u(t),\dot{u}_n(t))-\mathcal{L}(t,u(t),\dot{u}(t))|&\leq C\left(c(t)+\varphi(\chi_{\mathfrak{G}}|\dot{u}_n(t)|+
 \chi_{\mathfrak{G}^c}|\dot{u}(t)|)\right)|\dot{u}_n-\dot{u}|\\
 &=C\left(c(t)+\varphi(\chi_{\mathfrak{G}}|\dot{u}_n(t)|)+\varphi(
 \chi_{\mathfrak{G}^c}|\dot{u}(t)|)\right)|\dot{u}_n-\dot{u}|\\
 &\leq C\left(c(t)+\varphi(|\dot{u}_n(t)|)+\varphi(
 |\dot{u}(t)|)\right)|\dot{u}_n-\dot{u}|.
 \end{split}
 \]
 
 Now, in virtue of \cite[Lemma 9.1]{KR}, \cite[Lemma 17.1]{KR}, \cite[Theorem 17.4]{KR} and the uniform boundedness of $\dot{u}_n$ in 
 $\lphi$ we have
 \[|H(u_n)-H(u)|\leq C\|\dot{u}_n-\dot{u}\orlnor\to 0.\]
