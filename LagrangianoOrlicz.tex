...
Lemma \ref{inclusion orlicz} c) establish that $w(s):=s\Phi^{-1}(1/s)$ is a modulus of continuity for all functions $\b{u}\in\wphi$. We note that since $\Phi$ is a $N$-function we have $w(0)=0$ and from the concavity of $\Phi^{-1}$ we have that $w$ is increasing.
\end{comentario}
...
\end{split}
\end{equation}
Where the last equality was obtained from \cite[Eq. (9.11)]{KR}.

Inequality a) follows from c). In fact, since $\b{u}$ is continuous, from the Mean Value Theorem for integrals, there exists $s\in [0,T]$ such that
$\b{u}(s)=\overline{\b{u}}$. Using this $s$ value in c) and taking account that $s\Phi^{-1}(1/s)$ is increasing we obtain  Wirtinger's inequality. 
