...
there exists \linebreak $s_i\in [0,T]$ such that $u_i(s_i)=\overline{u}_i$.
...
Inequalities \eqref{in-sob-cont}  establish the embedding  $\wphi_d\hookrightarrow C([0,T],\rr^d)$. Actually a stronger result holds.  


 \begin{lem}\label{unif_conv} The embedding  $\wphi_d\hookrightarrow C([0,T],\rr^d) $ is compact. 
\end{lem}
\begin{proof} Let $\{\b{u}_{k}\}_{k \geq 1}$ be a bounded  sequence in $\wphi_d$. We have to prove that  there exists a subsequence $\{n_k\}$ and $\b{u}\in C([0,T],\rr^d)$ such that  $\{\b{u}_{k}\}_{k\geq 1}$ converges uniformly to $\b{u}$ on $[0,T]$.
...
\end{split}
\end{equation}
As $\Phi$ is an $N$-function, $\lim_{\xi\to \infty} \Phi^{-1}(\xi)/\xi=\lim_{\eta\to \infty} \eta/\Phi(\eta)=0$.  This fact and \eqref{equicont} prove that the sequence $\{\b{u}_{k}\}_{k \geq 1}$ is equicontinuous. By Arzel\`a-Ascoli Theorem, from every subsequence $\{k'\}\subset \mathbb{N}$ we can extract a subsequence $\{k''\}\subset\{k'\}$ such that $\b{u}_{k''}$ converges uniformly to a function $\b{v}\in C([0,T],\mathbb{R}^n)$. In particular $\b{u}_{k''}\rightharpoonup \b{v}$ in $\lphi_d$. However, from the embedding  $\wphi\hookrightarrow\lphi$, we have $ \b{u}_{k''}\rightharpoonup \b{u}$ in $\lphi$.  Thus, by the uniqueness of the weak limit, we get $\b{u}_{k''}\rightharpoonup \b{u}$ uniformly. 

{\color{red} Lo que sigue est\'a dicho arriba, o no?...
We have proved that from every subsequence $\{k'\}\subset \mathbb{N}$ we can extract a subsequence $\{k''\}\subset\{k'\}$ such that $\b{u}_{k''}\to \b{u}$ uniformly.}
...
Consequently, as $v \in \lphi_d$ and employing H\"older's inequality, we obtain that 
 $\sup_n|D_y\mathcal{L}(\cdot,\b{u}_n,\b{\dot{u}}_n)\ccdot v| \in L^1_1$.  Finally, from the Lebesgue Dominated Convergence Theorem, we deduce
...
\[
|\langle I'(\b{u}),\b{v} \rangle| \leq \|D_x\mathcal{L}\|_{L^1} \| \b{v}\linf + 
\|D_y\mathcal{L}\|_{\lpsi} \|\b{\dot{v}}\orlnor \leq C \|\b{v}\sobnor
\]
...
\end{equation}
where $\alpha_0,\Lambda>0$ and  $F:\rr\times\rr^d\to\rr$ is a Carath\'eodory function, i.e. $F(t,\b{x})$ is  measurable with respect to $t$ for every fixed  $\b{x}\in\rr^n$ and it is continuous at $\b{x}$ for a.e. $t\in [0,T]$. We note that, in virtue of \eqref{cota_inf} and \eqref{cotaL}, $F(t,\b{x})\leq a(|\b{x}|)b_0(t)$  with $b_0(t):=b(t)+\Phi(f(t))\in L^1_1([0,T])$. In order to ensure that integral $\int_0^TF(t,\b{u})dt$ is finite for $\b{u}\in\wphi$,  we need to assume 
...
\color{red}{con qu\'e?}
Alternativas: which contradicts the hypohtesis on $J_{C,1}$; which contradicts the coercivity  of $J_{C,1}$.
\end{proof}
...
The objectives of this section are i) to obtain conditions 
which ensure lower semicontinuity of the action integral $I$ on its domain $\domi$ and ii) to discuss the existence of minima for $I$.


This fact implies the next lemma.
\end{proof}
...
We also assume that the function $F$ satisfies inequality \eqref{condA1} and $\mathcal{L}(t,\b{x},\cdot)$ is convex in $\rr^d$ for each $(t,\b{x})\in [0,T]\times\rr^d$.  Then, the functional $I$  is sequentially weakly lower semicontinuous and  weakly* lower semicontinuous on $\wphi$.
\end{thm}

\begin{proof} We note that \eqref{cota_inf_2}
\end{proof}

{\color{red} Criterio \'unico para las referencias: Nombres completos o iniciales}
