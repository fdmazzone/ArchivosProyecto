...
We define the Sobolev-Orlicz space $\wphi_n$ (see \cite{adams_sobolev}) by
...
\end{split}
\end{equation}
Here we have used H\"older inequality (ver \cite[Th 9.3]{KR} ) and the formula for the norm of a characteristic function (ver  \cite[Eq. 9.11]{KR}). Inequality \eqref{desigualdad1} proves Wirtinger's inequality \eqref{wirtinger}. 
...
\[
\langle v, I'(\b{u})\rangle= \int_0^T \left\{D_{\b{x}}\mathcal{L}\big(t,\b{u}(t),\b{\dot{u}}(t)\big)\ccdot \b{v}(t)+ D_{\b{y}}\mathcal{L}\big(t,\b{u}(t),\b{\dot{u}}(t)\big)\ccdot\b{\dot{v}}(t)\right\} \ dt.
\]
