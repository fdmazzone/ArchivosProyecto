...
%%p.2 Agregue hipotesis b,c,f
%%p.4 Agregue referencia a holder
%p. 5 saque u en lphi
%lagrangian->Lagrangian en varios lados
% para L Caratheodory-> differentiable caratheodory (varios lados)
% p.10 caratheodory-> continuous differentiability
%p.17 Lemma 6.1 differentiable caratheodory. F condicion A
% 
...
\begin{proof} In the subsequent estimates, we use \eqref{cota_inf}, the decomposition $\b{u}=\b{\overline{u}}+\b{\tilde{u}}$, H\"older's inequality and Sobolev's inequality. 
...
Based on \cite{mawhin2010critical} we say that $F$ satisfies the condition (A) if  $F(t,\b{x})$ is a Carath\'eo\-dory function, $F$ verifies \eqref{condA1} and $F$ is continuously differentiable with respect to $\b{x}$. Moreover, the next inequality holds 
...
\begin{proof}  Using \eqref{cota_inf}, \cite[Eq. (18), p.17]{mawhin2010critical},  the decomposition $\b{u}=\b{\overline{u}}+\b{\tilde{u}}$,  \eqref{der_cero}, \eqref{holder} and Sobolev's inequality, we get
%
...
One of the main novelties of our work is that we obtain existence of solutions for lagrangian functions $\mathcal{L}(t,\b{x},\b{y})$ where the  nonlinearity is not a power function. 
And, in virtue of the fact that we have not supposed that the $N$-functions $\Phi$ satisfies the $\Delta_2$ condition, the nonlinearity is not even bounded by power functions.
For example, our main theorem can be applied to Lagrangians like
  \[\mathcal{L}(t,\b{x},\b{y})=e^{|\b{y}|}+F(t,\b{x}),\] 
when $F$  satisfies some of the alternatives 1-4 in the main theorem.
In this case, we can consider the $N$-function $\Phi(x)=e^x-x-1$ that verifies  conditions (2),(3) and (4)  and its complementary 
function is $\Psi(x)=(1+x)\log(1+x)-x$ that satisfies the $\Delta_2$ condition. We can apply our main theorem even if $\Psi$ are not a $\Delta_2$ function, although in this case we need an additional condition on the size of certain constants. 
...
\end{equation}
with  $g(t)\in L^1$, considered by Mawhin and Willem (see \cite[Th.1.5]{mawhin2010critical}). 
We point out that Mean Value Theorem for derivatives and inequality \eqref{eq:hip_ma_whi} imply our condition (38). 
In a series of papers (see for example  \cite{wu1999periodic,zhao2004periodic,tang2010periodic}) some weaker conditions than \eqref{eq:hip_ma_whi} for 
Lagrangians of the power type, such as
 \begin{equation}\label{eq:la_pot}\mathcal{L}(t,\b{x},\b{y})=|\b{y}|^p+F(t,\b{x}),\quad p>1,
 \end{equation}
were treated. In the aforementioned papers, inequality \eqref{eq:hip_ma_whi} was replaced by the weaker condition 
 \begin{equation}\label{eq:sublinear}|
\nabla F(t,\b{x})|\leq b(t)|x|^{\mu-1}+b_2(t),\quad b_1,b_2\in L^1, \mu\geq 1.
 \end{equation}
Furthermore, either $\mu$ is subcritical, that is $\mu<p$; 
or, assuming additional conditions on $b_1$, $\mu$  is critical, i.e. $\mu=p$.  
Our condition (38) is different to \eqref{eq:sublinear}, because of this fact our results are new even for 
lagrangian functions like \eqref{eq:la_pot}. 
For example, let $f(x)$ be a function defined on $\rr$, continuously  differentiable with  $f(x)=|x|$, for $|x|>1$; and, we define $F(t,x)=g(t)(f(x)+\cos(e^x))$, where  $g$ is any function in $L^1$. Then $F$ satisfies (38) because 
\[|F(t,x_1)-F(t,x_2)|\leq |g(t)|(K|x_1-x_2|+2),\]
since $f$ is a Lipschitz function. In addition, as $f(x)=|x|$ for $|x|>1$ and $\cos(e^x)$ is bounded, the function $F$ satisfies the coercivity condition (39). 
From here, our main theorem  can be applied to this function $F$.  
However,  $\frac{d}{dx}F=g(t)(f'(x)-e^x\sin(e^x))$ is not bounded 
by any power of $x$, unless $g$ is trivial, i.e. $F$ does not  satisfy \eqref{eq:sublinear}.

Alternatively, we consider the hypothesis of convexity on $F$. We would like to emphasize that a convex function does not necessarily 
satisfy (38). This observation is justified by the following fact: assume that $F$ is independent of $t$ and it also satisfies (38),
then $F$ is sublinear, i.e.
\[|F(x)|\leq a|x|+b,\quad a>0,b>0.\]
In fact, inequality (38) implies that 
$|F(x)-F(y)|\leq c$ with $c>0$ provided that  $|x-y|\leq 1$.
Now, if $x\in\rr^n$ we look for some $n\in\nn$ such that $n-1\leq |x|<n$. Therefore,
\[
\begin{split}
...
\end{split}
\]
Particularly, the function $F(t,x)=|x|^2$ is convex and satisfies the condition  (A); 
nevertheless,  $F$ is not sublinear and consequently it does not satisfy (38). 
The same function allows us to see that our condition (38) does not imply \eqref{eq:sublinear}.  
And condition (38) does not imply convexity either, which is clear taking the previously studied function  $F=g(t)(f(x)+\cos(e^x))$. 
