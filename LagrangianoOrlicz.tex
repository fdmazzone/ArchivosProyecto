...
\begin{lem}
We suppose that $L(t,x,y)$ is a Charateodory functions satisfying \eqref{cotaL}-\eqref{cotaDyL}. We suppose that $\Phi,\Psi\in\Delta_2$.  
...
We fix any $u\in\wphi$. What we must prove that for any sequence $\{u_n\}$ with $u_n\rightharpoonup u$ in $\wphi$ we have that $I(u)\leq \limsup_n I(u_n)$. We write
\[
...
\]
 ...
 \[J(u_n)=\left|  L(t,u_n(t),\dot{u}_n(t))-L(t,u(t),\dot{u}_n(t))\right|=|D_xL(t,\xi_n(t),\dot{u}_n(t))||u_n(t)-u(t)|.\]
...
\begin{equation}\label{acot_Dx}
 D_xL(t,\xi_n(t),\dot{u}_n(t))|\leq C \left(b(t)+\Phi(|\dot{u}_n(t))|)\right)
\end{equation}
...
\eqref{descomposicion}, \eqref{acot_Dx} and since $\|u_n-u\|\to 0$ and $b\in L^1$ we get $\|J(u_n)\|_{L^1}\to 0 	$

Now we will prove that $H(v)$ is w.l.s.c. Since $H(v)$ is convex it is sufficient to prove that $H$ is l.s.c (see
\cite[Theorem 1.2]{mawhin2010critical}). We suppose that  $\|u_n- u\|\to 0$. Then, there exists $s=s_{n,t}\in[0,1]$
such that
\[|H(u_n)-H(u)|=|D_yL(t,u(t),(1-s)\dot{u}_n(t)+s \dot{u}(t))||u_n-u|.\]
Let $\mathfrak{G}$ be the set $\{|u_n(t)|\leq |u(t)|\}$. Then
\[|(1-s)\dot{u}_n(t)+ \dot{u}(t)|\leq \max\{|\dot{u}_n(t)|,|\dot{u}(t)|\}=\chi_{\mathfrak{G}}|\dot{u}_n(t)|+
 \chi_{\mathfrak{G}^c}|\dot{u}(t)|
\]
Therefore, using \eqref{cotaDyL} and by the uniform boundedness of $a(|u_n(t)|)$ we get
\[\begin{split}|H(u_n)-H(u)|&\leq C\left(c(t)+\varphi(\chi_{\mathfrak{G}}|\dot{u}_n(t)|+
 \chi_{\mathfrak{G}^c}|\dot{u}(t)|)\right)|u_n-u|\\
 ...
 Now, in virtue of \cite[Lemma 18.2]{KR}, \cite[Lemma 18.2]{KR} and the uniform boundedness of $\dot{u}_n$ in 
 $\lphi$ we have
 \[\|H(u_n)-H(u)|_{\lpsi}\leq C\|u_n-u\orlnor\to 0.\]
 Which completes the proof (o eso quiero creer yo)
