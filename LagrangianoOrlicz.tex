...
(see \cite{mawhin2010critical, struwe2008variational}).  
...
\cite{tang1995periodic,tang1998periodic,tang2001periodic, xu2007some,tang2010periodic,ye2008periodic,zhao2004periodic,wu1999periodic}, 
the authors established conditions on Lagrangians defined on $L^p$ to guarantee coercivity and existence of minima.
For example, in   \cite{tang2001periodic} the Lagrangian is 
%\begin{equation}\label{lagrangiano_tang}
$\mathcal{L}(t,\b{x},\b{y})=\frac{|\b{y}|^2}{2}+F(t,\b{x}),$
%\end{equation}
where  $F(t,\b{x})\geq \beta(t)$ with $\beta\in L^1$ for every $\b{x}\in\mathbb{R}^d$ and a.e. $t\in[0,T]$. 
and such that $F(t,\b{x})\to+\infty$, a.e. $t\in E$ when $|\b{x}|\to+\infty$ being   $E\subset[0,1]$ with $|E|>0$.
Later, in  \cite{xu2007some} these requirements  were generalized to the Lagrangian 
%\begin{equation}\label{lagrangiano_p}
$\mathcal{L}(t,\b{x},\b{y})=\frac{|\b{y}|^p}{p}+F(t,\b{x}),$
%\end{equation}
for $p>1$.
And, in \cite{xu2007some,tang2010periodic,wu1999periodic,zhao2004periodic} the function $F$
