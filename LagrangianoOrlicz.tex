...
In Orlicz spaces a generalization of  \emph{H\"older inequality} holds (ver \cite[Th 9.3]{KR} ). Namely, for every $\b{u}\in\lphi_n$ and $\b{v}\in\lpsi_n$, $u\ccdot v\in L^1_1$ and
\begin{equation}\label{holder}
 \langle \b{u},\b{v}\rangle:=\int_0^T\b{u}\ccdot\b{v} dt\leq \|\b{u}\orlnor \|\b{v}\orlnor.
\end{equation}
H\"older inequality implies  that $\lpsi_n\subset \left[\lphi_n\right]^*$, where $\b{v}\in\lpsi$ induce the functional . In this direction, it is known that the space $\ephi_n$ satisfies the important relation  $\left[\ephi_n\right]^*=\lpsi_n$. 
...
We will use the following fact frequently 
...
On the other hand, using \cite[Eq. 9.11]{KR} and  \cite[Eq. 9.11]{KR} again, we obtain
...
\end{proof}
 ...
 Given a contiuous function $a\in C(\mathbb{R}^+,\mathbb{R}^+)$ and we define the composition operator $\b{a}:M_n\to M_n$ by $\b{a}(\b{u})(t)= a(|\b{u}(t)|)$.
We will use repeatedly the following elementary consequence of the previous lemma. 
\begin{cor}\label{a_bound} If $a\in C(\mathbb{R}^+,\mathbb{R}^+)$ then $\b{a}:\wphi_n\to L^{\infty}_1([0,T])$ is bounded, which is there exists a non decreasing function $c:\mathbb{R}^+\to\mathbb{R}^+$ such that
...
\begin{equation}\label{dergateauxI}
\langle v, I'(\b{u})\rangle= \int_0^T \left\{D_{\b{x}}\mathcal{L}\big(t,\b{u},\b{\dot{u}}\big)\ccdot \b{v}+ D_{\b{y}}\mathcal{L}\big(t,\b{u},\b{\dot{u}}\big)\ccdot\b{\dot{v}}\right\} \ dt.
\end{equation}
...
\noindent\textbf{Step 2.} We will prove that the mapping  $\b{u}
 \mapsto  D_{y}\mathcal{L}(t,\b{u},\b{\dot{u}})$ is continuous from $\domi$ with the strong topology  into $\left[\lphi_n\right]^*$  with the weak$^*$ topology. Let $\b{u}\in \domi$.  It follows from  \eqref{inclusion3}, Lemma \ref{phi_comp} and Corollary \ref{a_bound} that $\varphi\left(|\b{u}|/\lambda+d\right)\in C^{\Psi}_1$ and $\b{a}(|\b{u}|)\in L^{\infty}_1$. Therefore, in virtue of  \eqref{cotaDyL} we get
\begin{equation}\label{DyLpsi}
   \left|D_y\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))\right|\leq  c(\|\b{u}\|_{\wphi} )  \left(c+\boldsymbol{\varphi}\left( |\b{u}|/\lambda+d\right  ) \right)\in\lpsi.
\end{equation}
Hence $D_y\mathcal{L}(t,\b{u},\b{\dot{u}})\in L^{\Psi}_n\subset\left[\lphi_n\right]^*$. This last observation  and the fact that  $D_x\mathcal{L}(t,\b{u},\b{\dot{u}})\in L^1_n$ and 

Now, let us to prove the continuity of the map   $\b{u}\mapsto D_y\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))$. We take $\b{u}_n,\b{u}\in \domi$ with $\b{u}_n\to \b{u}$ in the norm of $\wphi_n$. We must prove that  $D_y\mathcal{L}(t,\b{u}_n(t),\dot{\b{u}_n}(t))\overset{w^*}{\rightharpoonup} D_y\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))$. Suppose, on the contrary, that there exists $\b{v}\in\lphi$, $\epsilon>0$ and a subsequence of $\{\b{u}_n\}$ (again denoted for simplicity $\{\b{u}_n\}$)  such that
\begin{equation}\label{cota_eps}
 \left| \langle \b{v}, D_y\mathcal{L}(t,\b{u}_n(t),\b{\dot{u}_n}(t)) \rangle - \langle \b{v}, D_y\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))\rangle\right|\geq \epsilon.
\end{equation}
