...
For reader convenience, a short introduction to Orlicz and Orlicz Sobolev spaces of vector valued functions is given. We refer to \cite{adams_sobolev,KR} for additional details and proofs. In these books is considered scalar valued function, however the generalization of the results enumerated below  to vector valued functions is direct.
...
For  $n$  positive integer we denote by $M_n:=M_n([0,T])$ the set of all measurable functions defined in $[0,T]$ with values in $\mathbb{R}^n$.  In this paper we adopt the convention of to use bold symbols for denote $\mathbb{R}^n$-valued function and plain symbols for scalar ones. Given  a $N$-function $\Phi$ we define the \emph{modular function} $\rho_{\Phi}:M_n\to [0,+\infty]$ by
