...
\begin{nota_fer}
Me parece que vamos a tener que presentar el resultado siguiente de otra forma. Como está escrito, resultará un caso particular de otros teoremas. Tener en cuenta que, al fin y al cabo, se termina pidiendo que $\Phi,\Psi\in\Delta_2$, con lo cual se aplicarían los resultados que escribe Sonia más abajo y la funcional sería coercitiva sin necesidad de la hipótesis \eqref{cota}.  Para poder mantener el resultado de abajo, habría que contextualizarlo en un teorema donde no se pida  $\Phi,\Psi\in\Delta_2$. Se me ocurre que un buen lugar sería después del comentario que sigue a la estimación \eqref{eq:no_coerciva}. Pues allí se habla del papel que juega la constante $C$ y me parece bueno relacionarlo con eso.

Todo el razonamiento hasta llegar a la estimación \eqref{eq:est_abajo_I} se puede tomar para usarlo con lo que escribió Sonia. Justamente lo que demostró Sonia es que la cota inferior en  \eqref{eq:est_abajo_I} tiende a infinito cuando $\|\b{\dot{u}}\orlnor\to\infty$.\textbf{ Eso si, habría que intetar generalizar este razonamiento, sin pedir la condición $\mathcal{L}(t,\b{x},\b{y})=\Phi(|\b{y}|)+F(t,\b{x})$.} Algo habrá que pedirle a $\mathcal{L}$, intentar que sea alguna desigualdad, en lugar de adjudicarle una forma más explícita. 
