...
\begin{proof} Let $\{\b{u}_n\}\subset \wphi_T$ be a  minimizing sequence for the problem  $\min_{\b{u}\in\wphi_T}I(\b{u})$.
Since  $I(\b{u}_n)$, $n=1,2,\ldots$  is bounded, Theorem \ref{coercitividad1} imply that $\{\b{u}_n\}$ is norm bounded in $\wphi_d$. In virtue of Corollary \ref{unif_conv} we can assume that $\b{u}_n$ converges uniformly to a $T$-periodic continuous function $\b{u}$.  The space  $\lphi_d$ is a predual space, concretely $\lphi_d=\left[\epsi_d\right]^*=\left[\lpsi_d\right]^*$. Therefore, according to \cite[Prop. 4, p. 147]{rao1991theory} (note that $\Phi\in\nabla_2\Leftrightarrow \Psi\in\Delta_2$) and since $\b{\dot{u}}_n$ is bounded in $\lphi_d$,  there exists a subsequence (again denoted $\b{u}_n$) such that $\b{\dot{u}}_n$ converges to a function $\b{v}\in\lphi_d$ in the weak* topology. From this fact and the uniform convergence of $\b{u}_n$ to $\b{u}$ we obtain for every $T$-periodic function $\b{\xi}\in C^{\infty}([0,T],\rr^n)$
...
Finally from the relations $\left[L^1_d\right]^*=L^{\infty}_d\subset  \lpsi$ and $\lphi_d\subset L^1_d$ we obtain that $\b{\dot{u}}_n$ converges to $\b{\dot{u}}$ in the weak topology of $L^1_d$. Consequently Theorem \ref{semicontinf} implies 
\[I(\b{u})\leq  \liminf_{n\to\infty}I(\b{u}_n)=\min\limits_{\b{u}\in\wphi_T}I(\b{u}).\]
Hence $\b{u}$ is a minimun, and therefore a critical point, of $I$. Applying Theorem \ref{critpoint} we conclude the proof.\end{proof}
