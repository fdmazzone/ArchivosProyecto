...
\end{equation}
where $\alpha_0,\Lambda>0$ and  the function $F:\rr\times\rr^d\to\rr$ is measurable with respect to $t$ for every fixed  $\b{x}\in\rr^d$ and continuously differentiable at $\b{x}$ for a.e. $t\in [0,T]$. We note that, in virtue of \eqref{cota_inf} and \eqref{cotaL}, $F(t,\b{x})\leq a(|\b{x}|)b_0(t)$, with $b_0(t):=b(t)+\Phi(d(t))\in L^1_1([0,T])$. In order to ensure that the integral $\int_0^TF(t,\b{u})\ dt$ is finite for $\b{u}\in\wphi_d$,  we need to assume 
