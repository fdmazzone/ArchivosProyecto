...
which ensure lower semicontinuity of the action integral $I$ on its domain $\domi$ and ii) discuss the existenceofminima for $I$.
...
\end{lem}
\begin{proof}
 Since $\b{u}_{k}\rightharpoonup \b{u}$ in $\wphi$,  $\{\b{u}_{k}\}_{k \geq 1}$ is bounded in
$\wphi$. Therefore $\{\b{\dot{u}}_n\}$   is bounded in $\lphi$. For $0 \leq
s\leq t \leq T $, we have
\begin{equation}\label{equicont}
\begin{split}
\left|\b{u}_{k}(t)-\b{u}_{k}(s) \right| &\leq \int_{s}^t \left| \dot{\b{u}}_{k}(\tau)\right|\ \ d\tau\\
&\leq \| \chi_{[s,t]}\|_{\lpsi}\|\dot{\b{u}}_{k}\|_{\lphi}\\
&\leq C ( t-s)\Phi^{-1}\left(\frac{1}{t-s}\right).
\end{split}
\end{equation}
As $\Phi$ is $N$-function, $\lim_{\xi\to \infty} \Phi^{-1}(\xi)/\xi=\lim_{\eta\to \infty} \eta/\Phi(\eta)=0$.  This fact and \eqref{equicont} prove that the sequence $\{\b{u}_{k}\}_{k \geq 1}$ is equicontinuous. By Arzela-Ascoli theorem, $\b{u}_{k}$ converges uniformly to a function $\b{v}\in C([0,T],\mathbb{R}^n)$. In particular $\b{u}_{k}\rightharpoonup \b{v}$ in $\lphi$. From the immersion $\wphi\hookrightarrow\lphi$ we have $ \b{u}_{k}\rightharpoonup \b{u}$ in $\lphi$.  By the uniqueness of the weak limit in $\b{u}_{k}\rightharpoonup \b{u}$ uniformly.
\end{proof}
