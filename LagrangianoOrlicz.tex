...
\begin{comentario}Inequalies \eqref{in-sob-cont} and \eqref{wirtinger}  proves that the embedding  $\wphi_d\hookrightarrow C^w([0,T],\rr^d)$ holds, where $C^w([0,T],\rr^d)$ denotes the space of generalized $w$-H\"older continuous functions. This is the space of all functions satisfying \eqref{w-holder} for some $C>0$ and $w(s)=s\Phi^{-1}(1/s)$ and it is a Banach space with norm
\[\|\b{u}\|_{  C^w([0,T],\rr^d) }  :=\|\b{u}\|_{L^{\infty}}+\sup\limits_{t\neq s}\frac{|\b{u}(t)-\b{u}(s)|}{w(|t-s|)}.\]
As it is well known, Arzela-Ascoli Theorem implies that  the embedding $C^w([0,T],\rr^d)\hookrightarrow C([0,T],\rr^d)$ is compact (see \cite[Ch. 5]{driver} for the case $w(s)=|s|^{\alpha}$, $0< \alpha\leq 1$ and if  $w$ is arbitrary, the proof follows with some modifications). Therefore we have the following result.
\end{comentario}
...
{\color{red} Comentamos el Remark 3 porque pensamos que debería escribirse en el texto, como lo hicimos debajo}
%\begin{comentario}These conditions are a direct  generalization of the conditions
%\cite[Eq (a), p. 10]{mawhin2010critical}. 
%\end{comentario}

In the following comments we discuss the relevance of the function $f$ in the inequalities \eqref{cotaL},  \eqref{cotaDxL} and \eqref{cotaDyL}. These conditions are a direct  generalization of the conditions \cite[Eq (a), p. 10]{mawhin2010critical}.  In particular, we are interested in seeing when for every  $f\in \ephi_1$ there exist
$b\in L^1_1$ {\color{red}o $\widetilde{b}$, para que no coincida con la ecuacion (2) y (3)}  and a constant $C>0$ such that 
\begin{equation}\label{cotadb}
\Phi(s+f(t))\leq C\Phi(s)+b(t)\;\;\mbox{for every}\;s>0.
\end{equation} 
{\color{red}If \eqref{cotadb} is true then in the equations \eqref{cotaL} and \eqref{cotaDxL} we can suppose $f=0$}. The same considerations should be done with $\varphi\left(s+f(t)\right)$.
...
\begin{comentario} If $\Phi\in\Delta_2$ then we can assume $f=0$ keeping the same value of $\lambda$. This is a consequence of the fact that a non decreasing function $G:\mathbb{R}^+\to\mathbb{R}^+$ satisfying the $\Delta_2$-condition, is a quasi-subadditive function (see \cite[Prop. 4.2]{AF12}) i.e. there exists $K>0$ such that 
...
This fact proves item \ref{T1item1}.
 ...
 We split the proof of item \ref{T1item3} in four steps.
...
{\color{red}\emph{Step 4.} (agregamos este paso)} The operator $I':\domi  \to \left[\wphi_d
\right]^* $ is demicontinuous. This is a consequence of the continuity of the mappings $\b{u} \mapsto D_{\b{x}}\mathcal{L}(t,\b{u},\b{\dot{u}})$ and $\b{u} \mapsto
...
In order to prove item  \ref{T1item4}, it is necessary to see that the maps $\b{u}\mapsto D_{\b{x}}\mathcal{L}(\cdot,\b{u}(\cdot),\b{\dot{u}}(\cdot))$  and $\b{u}\mapsto D_{\b{y}}\mathcal{L}(\cdot,\b{u}(\cdot),\b{\dot{u}}(\cdot))$  are norm continuous
from $\domi $ into $L^1_d$ and
 $\lpsi_d$ respectively {\color{red} Debemos unificar la notacion respecto a la usada en el item 2 $\b{u}\mapsto D_{\b{x}}\mathcal{L}(\cdot,\b{u}(\cdot),\b{\dot{u}}(\cdot))$ o $\b{u}\mapsto D_{\b{x}}\mathcal{L}(t,\b{u},\b{\dot{u}})$ ?}.  The continuity of the first map has already been proved in step 1. 
...
In this section we derive the Euler-Lagrange equations associated to critical points of action integrals.  We denote by $\wphi_T$ the subspace of $\wphi_d$ containing all \linebreak $T$-periodic functions. As usual, when $Y$ is a subspace of
...
For the lower semicontinuity of  $I$ we only need to adapt some results of \cite{ekeland1999convex} to our problem. {\color{red} Este parrafo introductivo deberia hacer mejor propaganda del resultado principal}
...
Let $\{\b{u}_n\}$ be a sequence  satisfying the assumptions of the theorem.   We define the Carath\'eodory function $\mathcal{\hat{L}}=\mathcal{L}-F$. Let $\hat{I}$ be the associated to $\mathcal{\hat{L}}$ action integral. Using  \cite[Thm. 2.1, p. 243]{ekeland1999convex} with  $\mathcal{\hat{L}}$ instead $f$, we have that
...
\begin{thm} We suppose $\Phi$ and $\Psi$ complementary $N$-functions. We assume that the Carath\'eodory function $\mathcal{L}(t,\b{x},\b{y})$ is strictly convex in $\b{y}$, $D_{\b{y}}\mathcal{L}$ is $T$-periodic with respect to $T$  and that it satisfies \eqref{cotaL}, \eqref{cotaDxL}, \eqref{cotaDyL}, \eqref{cota_inf} and  \eqref{condA1}. In addition, we suppose that some of the following statements hold (we recall the definitions and properties of $\alpha_0$, $b_1$, $\b{x}_0$ and $b_0$ from \eqref{cota_inf}, \eqref{holder_cont}, \eqref{der_cero} and \eqref{condA2} respectively)
\begin{enumerate}
...
Since  $I(\b{u}_n)$, $n=1,2,\ldots$  is bounded {\color{red} o es acotada solo por arriba?, el espacio sobre el que minimizamos no excluye la posibilidad de $-\infty$. De todas maneras solo con acotacion superior se tiene el mismo resultado}, Theorem \ref{coercitividad1} (or Theorem \ref{segcoerthm} according to which of the items \ref{item1prin}-\ref{item4prin} hold true) implies that $\{\b{u}_n\}$ is norm bounded in $\wphi_d$. Hence, in virtue of Corollary \ref{unif_conv} we can assume that $\b{u}_n$ converges uniformly to a $T$-periodic continuous function $\b{u}$.  The space  $\lphi_d$ is a predual space, concretely $\lphi_d=\left[\epsi_d\right]^*$. Therefore, from \cite[Cor. 5, p. 148]{rao1991theory} and since $\b{\dot{u}}_n$ is bounded in $\lphi_d$,  there exists a subsequence (again denoted $\b{\dot{u}}_n$) such that $\b{\dot{u}}_n$ converges to a function $\b{v}\in\lphi_d$ in the weak* topology. From this fact and the uniform convergence of $\b{u}_n$ to $\b{u}$ we obtain for every $T$-periodic function $\b{\xi}\in C^{\infty}([0,T],\rr^d)\subset\epsi_d$
\[
