...
\begin{comentario}  As a direct consequence of convexity, we can  bound the term $\Phi(|\b{y}|/\lambda+d(t))$, $d\in \ephi_1$, by the expression  $\frac12\Phi(|\b{y}|/\Lambda)+b(t)$ where $b(t):=\tfrac12\Phi(2d(t))\in L^1_1$, and  $\Lambda= \lambda/2$. That is, we can  assume $d = 0$ at the price of making smaller the value of $\lambda$. 
\end{comentario}

  


\begin{comentario} Let us note that if $\Phi\in\Delta_2$ then we can asssume $d=0$ keeping the same value of $\lambda$. This is consequence of that a non decreasing $\Delta_2$ function $G:\mathbb{R}_+\to\mathbb{R}_+$ is quasi-subadditive. In fact, we suppose $y\leq x$, then  
...
Here we have used \cite[Th. 4.1]{KR}, the $\Delta_2$ condition for $\Phi$ and the inequality $\Phi(x)\leq x\varphi(x)$ valid for any $N$-function. Therefore if $\Phi$ is $\Delta_2$ we have that
...
\begin{equation}\label{DyLpsi}
   \left|D_{\b{y}}\mathcal{L}(t,\b{u}(t),\b{\dot{u}}(t))\right|\leq  c(\|\b{u}\|_{\wphi} )  \left(c+\boldsymbol{\varphi}\left( \frac{|\b{u}|}{\lambda}+d\right  ) \right)\in\lpsi_1.
\end{equation}
...
\emph{Step 3.} Finally we prove \ref{T1item3}. The proof follows similar lines that \cite[Theorem 1.4]{mawhin2010critical}. For $\b{u}\in \domi$ and $\b{v}\in\wphi_n$ we define the function
...
\]
As a consequence $\b{\dot{u}}+s_0\b{\dot{v}} \in \Pi(\ephi_n,\lambda)$ and  $|\b{\dot{u}}|+s_0|\b{\dot{v}}| \in \Pi(\ephi_1,\lambda)$. These facts imply, in virtue of Theorem \ref{teorema_acotacion}(\ref{T1item1}) that $I(\b{u}+s\b{v})$ is well defined and it is finite for $|s|\leq s_0$. 
...
\end{split}
\end{equation}
 Invoking \eqref{DxL1}, \eqref{DyLpsi} with $ |\b{\dot{u}}|+s_0|\b{\dot{v}}|$ instead $\b{u}$ and taking account of $\b{\dot{v}}\in L^{\infty}$ and $\b{v}\in\lphi$ we show that there exists a function $g \in L^1_1([0,T], \mathbb{R}^{+})$
such that $|D_s f(s,t)| \leq g(t)$. Consequently, $I$ has a directional derivative and
\[
\langle v, I'(\b{u}) \rangle=\frac{d}{ds}I(\b{u}+sv)\big|_{s=0}=\int_0^T  D_{\b{x}}\mathcal{L}(t,\b{u},\b{\dot{u}})\ccdot \b{v}+ D_{\b{y}}\mathcal{L}(t,\b{u},\b{\dot{u}})\ccdot\b{\dot{v}}  dt.
\]
...
By Lemma \ref{segundo lema}, there exist a subsequence $\b{u}_{n_k}$ and $h\in \domi$ such that $\b{u}_{n_k}\rightarrow \b{u} \quad\text{a.e.}$, $\b{\dot{u}}_{n_k}\rightarrow \b{\dot{u}} \quad\text{a.e.}$ and $|\b{\dot{u}}_{n_k}|\leq h\quad\text{a.e.}$. Then  since $\mathcal{L}$ is a Caratheodory function
...
In this section we derive the Euler-Lagrange equations associated to critical points of action integrals.  We denote by $\wphi_T$ the subspace of $\wphi$ of all  $T$-periodic functions. Similarly we consider the subspaces $\ephi_T$, $\lphi_T$. As is usual, when $Y$ is a subspace of
...
\begin{enumerate}
  \item If $C\Lambda<1$ then $J_{C,1}$ is coercitive. 
...
\end{equation}
In particular, the functional $J_{C,\mu}$ is coercitive for every $C>0$ and  $0<\mu<a_{\Phi}$. The constant $\alpha_{\Phi}$ is one of the so called \emph{ Matuszewska-Orlicz indices} (see \cite[Ch. 11]{M}).
\item If $J_{C,1}$ is coercitive with $C\Lambda>1$, then $\Psi$ is $\Delta_2$ at infinity.  
\end{enumerate}
\end{lem}

\begin{proof} Using \eqref{amemiya} we obtain
\[(1-C\Lambda)\|\b{u}\orlnor+C\Lambda\|\b{u}\orlnor\leq \Lambda +\Lambda \rho_{\Phi}\left(\frac{\b{u}}{\Lambda}\right).\]
Then
...
\end{split}
\]
We choose $r=\|\b{u}\orlnor/2$. Since $\|\b{u}\orlnor\to+\infty$   we can assume $\|\b{u}\orlnor>2$.  Thus $r>1$ and 
...
In order to prove the last item, we assume that $\Psi \notin \Delta_2$ at infinity. By \cite[Th. 4.2]{KR},  there exists a sequence of real  numbers  $r_n$ such that
$r_n \to \infty$ and 
...
\item\label{hipot_coer}  $\Psi$ is $\Delta_2$ or 
\begin{equation}\label{cotaC}a_0^{-1}T\Psi^{-1}\left(\frac{1}{T}\right)\|b_1\|_{L^1}<1.
\end{equation}
\end{enumerate}
...
\begin{proof} In the following estimates we will use the descomposition $\b{u}=\b{\overline{u}}+\b{\tilde{u}}$, H\"older and the Wirtinger inequality \eqref{wirtinger}. 
\begin{equation}\label{cota_inf_I}
\begin{split}
I(\b{u})&=\int_{0}^T \mathcal{L}(t,\b{u}(t),\dot{\b{u}}(t))\ dt\\
&\geq a_0\int_0^T\Phi\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)+F(t,\b{u})dt\\ 
&=a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)+ \int_0^TF(t,\b{u})-F(t,\b{\overline{u}})dt +  \int_0^TF(t,\b{\overline{u}})dt\\
&\geq a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)- \int_0^Tb_1(t)(1+|\b{\tilde{u}}(t)|^{\mu})dt +  \int_0^TF(t,\b{\overline{u}})dt\\
&\geq a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)- \|b_1\|_{L^1}(1+\|\b{\tilde{u}}\|_{L^{\infty}}^{\mu}) +  \int_0^TF(t,\b{\overline{u}})dt\\
&\geq a_0\rho_{\Phi}\left( \frac{|\b{\dot{u}}|}{\Lambda}\right)- \|b_1\|_{L^1}\left(1+\left[T\Psi^{-1}\left(\frac{1}{T}\right)\right]^{\mu}\|\b{\dot u}\orlnor^{\mu}\right) \\
&\quad+  \int_0^TF(t,\b{\overline{u}})dt\\
&=a_0J_{C,\mu}(\b{\dot{u}})- \|b_1\|_{L^1}+ \int_0^TF(t,\b{\overline{u}})dt,
\end{split}
\end{equation}
where $C=a_0^{-1}\left[T\Psi^{-1}\left(1/T\right)\right]^{\mu}\|b_1\|_{L^1}$.
...
\begin{thm}
We assume the same hypothesis that in Theorem \ref{coercitividad1}, except condition \eqref{holder_cont}. Instead we suppose that $F(t,\cdot)$ is convex and differentiable for a.e. $t\in [0,T]$ and that the following inequality  holds
\begin{equation}\label{condA2} |D_{\b{x}}F(t,\b{x})|\leq a(|\b{x}|)b_0(t),\quad\text{for a.e. }t\in [0,T] \text{ and all }\b{x}\in\rr^n,
\end{equation}
with $a$ and $b_0$ as in \eqref{condA1}. Then $I$ is coercive. 

\end{thm}
\begin{comentario}  Inequalities \eqref{condA1} and \eqref{condA2} imply that $F$ satisfies condition (A) (see \cite[p. 12]{mawhin2010critical}).  
\end{comentario}

\begin{proof} We note that the function $F$ is under the same hypothesis that in \cite[Th. 1.7]{mawhin2010critical}. Therefore, attending to \cite[p. 18, Eq. 20]{mawhin2010critical} we get $\b{x}_0$ such that 
\[
 \int_0^T \nabla F(t,\b{x}_0) dt=0.
\]


\[
\begin{split}
I(u_n)&=\int_{0}^T \Phi(|\dot{\b{u}}_n|)+F(t,{\b{u}}_n)  dt \\
&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+\int_0^T F(t,x_0) dt + \int_0^T \nabla F (t,x_0) \cdot (u_n-x_0) dt\\
&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+\int_0^T F(t,x_0) dt + \int_0^T \nabla F (t,x_0) \cdot (\overline{u}_n +\widetilde{u}_n -x_0) dt\\
&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+\int_0^T F(t,x_0) dt + \int_0^T \nabla F (t,x_0) \cdot \widetilde{u}_n dt+ \int_0^T \nabla F (t,x_0) \cdot (\overline{u}_n  -x_0) dt\\
&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+\int_0^T F(t,x_0) dt + \int_0^T \nabla F (t,x_0) \cdot \widetilde{u}_n dt+ 0.
\end{split}
\]
Donde $u_n = \overline{u}_n +\widetilde{u}_n $, con $\overline{u}_n =(1/T)\int_0^T u_n (t) dt$. Aplicando la desigualdad de Hölder

\[
\begin{split}
I(u_n)&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+\int_0^T F(t,x_0) dt -\| \nabla F (t,x_0)\|_1 
\| \widetilde{u}_n\|_{\infty}\\
&\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt-c_1 -c_2 \|\dot{u}_n  \|_{\Phi} .
\end{split}
\]
Como $\Psi$ es $\Delta_2$ entonces $\int \Phi(\dot{u}_n|)\,dx-C\|\dot{u}_n{\orlnor} \to \infty$ cuando $\|\dot{u}_n{\orlnor} \to \infty$.
Esto implica que existe una constante $c_3$  tal que $\|\dot{u}_n{\orlnor} \leq c_3$. Como el promedio de $\widetilde{u}_n$ es cero, se tiene
\begin{equation}\label{cotatilde}
\| \widetilde{u}_n\|_{\infty} \leq c \| \dot{\widetilde{u}}_n\|_{\Phi} = c(T)\| \dot{u}_n\|_{\Phi} \leq c_4.
\end{equation}

Por la convexidad de $F$
\[
F(t, \overline{u}_n /2)=F(t,(1/2)(u_n - \widetilde{u}_n)) \leq (1/2)F(t,u_n) + (1/2) F(t, -\widetilde{u}_n).
\]
Entonces,
...
\[
\begin{split}
I(u_n) &\geq \int_{0}^T \Phi(|\dot{\b{u}}_n|) dt+2 \int_0^T F(t,\overline{u}_n /2) dt - \int_0^T F(t, -\widetilde{u}_n) dt\\
& \geq 0 +2 \int_0^T F(t,\overline{u}_n /2) dt - \int_0^T F(t, -\widetilde{u}_n) dt.
\end{split}
\]
Por propiedad de acotación de la $F$: $( |F(t,x)|\leq a(|x|)b(t) )$ y ladesigualdad \eqref{cotatilde} se sigue que existe $c_5$ tal que
\[
\begin{split}
I(u_n) &\geq 2 \int_0^T F(t,\overline{u}_n /2) dt - c_5.
\end{split}
\]
Luego, por \eqref{propiedad1coercividad} se sigue que $\{\overline{u}_n \}$ esta acotado. Luego $\{u_n\}$ es acotada y así, toda sucesión minimizante resulta acotada.
