...
Another contribution that we want to emphasize is the condition \eqref{holder_cont}. This condition is a relaxed version of the inequality
\begin{equation}\label{eq:hip_ma_whi}
  |\nabla F(t,\b{x})|\leq g(t),
\end{equation}
with  $g\in L^1_1$, considered by Mawhin and Willem (see \cite[Th.1.5]{mawhin2010critical}). 
We point out that Mean Value Theorem for derivatives and inequality \eqref{eq:hip_ma_whi} imply our condition \eqref{holder_cont}. 
...
Our condition \eqref{holder_cont} is different to \eqref{eq:sublinear}, because of this fact our results are new even for 
...
since $f$ is a Lipschitz function. In addition, as $f(x)=|x|$ for $|x|>1$ and $\cos(e^x)$ is bounded, the function $F$ satisfies the coercivity condition \eqref{propiedad1coercividad}. 
...
satisfy (38). This observation is justified by the following fact: assume that $F$ is independent of $t$ and it also satisfies \eqref{holder_cont} ,
then $F$ is sublinear, i.e.
\[|F(x)|\leq a|x|+b,\quad a>0,b>0.\]
In fact, inequality \eqref{holder_cont} implies that 
...
nevertheless,  $F$ is not sublinear and consequently it does not satisfy \eqref{holder_cont}. 
The same function allows us to see that our condition \eqref{holder_cont} does not imply \eqref{eq:sublinear}.  
And condition \eqref{holder_cont} does not imply convexity either, which is clear taking the previously studied function  $F=g(t)(f(x)+\cos(e^x))$. 
