...
In the present paper we extend the analysis in....FALTA
...
r(t)\ddot{\theta}(t)+2\dot{r}(t)\dot{\theta}(t) & =0.\\
\end{array}
\end{equation}
...
\section{Admissible configurations}
...
\section{Admissible collisionless configurations for $n\leq 4$}
...
\section{Massless particle motion}
...
If the equation \eqref{eq:eq_new_red} has a $T$-periodic solution,  we say that the solution is synchronous. Like as \cite{li2013characterization, zhao2015nonplanar}, we are interesting in to find synchronous solutions.
...
\begin{thm}\label{thm:prin_ine} We assume that $q$ is a admisible collisionless configuration. Every solution of \eqref{eq:eq_new_red} if of the some following types:
\begin{enumerate}
...
\begin{enumerate}
 \item for $z_E$ the only positive solution of $-\sum_{i=1}^n m_i (s_i^2+z_{E}^2)^{-\frac12}=E$
 \[
 T_0(E)=2^{3/2}\int_{0}^{z_E} \left(E+\sum_{i=1}^n m_i (s_i^2+z^2)^{-\frac12}\right)^{-\frac12} dz,
 \]
  \item $T_0(E)$ is an increasing function.
 \item $T_0\left((E_{min},0)\right)=(T_{min},+\infty)$, where  $T_{min}=2\pi\left(\sum_{i=1}^n\frac{m_i}{s_i^3} \right)^{-1/2}$.
...
We note that $f(z_Eu,z_E)$ is a increasing function with respect to $z_E$ for $u\in [0,1]$ fix. This implies item \textit{2}.
...
Finally,  since $T_0=T_0(z_E)$ is continuous and increasing respect to $z_E$ we conclude the afirmation in the item \textit{3}.

PONER ALTERNATIVAS!!!!
\end{proof}


\begin{cor}\label{cor:sol.sincronica}
 There is a $T$-synchronous solution if and only if
...
\end{cor}
\begin{proof}
The result is consequence of Theorem \ref{thm:prin_ine} that $T^2=4\pi^2 \sum_{i=1}^{n}m_i|q_i|^2/U$   (see \cite[p. 109]{JaumeLlibre276}).
\end{proof}
...
We suppose that $q_1,\ldots,q_n$ is the equal masses regular polygon configuration  (this is an admisible CC). Then there exists a synchronous solution if and only if $2\leq n\leq 472$.
\end{cor}
...
In this section we present an example that shows the condition of non-stacionarity in Theorem \eqref{thm:prim} is needed.
...
\begin{equation}\label{eq:mov_rot}\ddot{\xi}+2\nu B\dot{\xi}+\nu^2 C\xi=\sum_{i=1}^n\frac{m_i(q_i-\xi)}{|q_i-\xi|^3},\end{equation}
where
\[B:=\begin{pmatrix}
       J & 0_{2\times 1}\\
       0_{1\times 2} & 0\\
     ...
     \end{pmatrix},
\]
where $0_{n\times m}$ and $\mathcal{I}_{n}$ denote the null $n\times m$ matrix  and the identity $n\times n$ matrix respectively. Assuming that the massless particle is moving on the $z$-axis then $\xi=q=(0,0,z)$ and the Coriolis and centrifugal forces,   $2\nu B\dot{\xi}$ and $\nu^2 C\xi$ respectively, are null. Therefore, taking account in the first two equation in \eqref{eq:mov_rot} and identifying the vectors $q_i$, $i=1,\ldots,n$ with vectors in $\rr^2$, we have
\[
\sum_{i=1}^n\frac{m_iq_i}{|q_i-\xi|^3}=0.
\]

Let $D=\{|q_i|: i=1,\ldots,n\}$.  Suppose that $D=\{r_1,\ldots,r_k\}$, with $r_i\neq r_j$ for $i\neq j$, and  $\{1,\ldots,n\}=F_1\cup \cdots\cup F_k$, where if $i\in F_j$ then $|q_i|=r_j$. Then
\[\sum_{j=1}^k\left\{\frac{1}{(r_j^{2}+z^2)^{3/2}}\sum_{i\in F_j}m_iq_i\right\}=0.\]
...
\[\sum_{j=1}^k\left\{\frac{1}{(r_j^{2}+s)^{3/2}}\sum_{i\in F_j}m_iq_i\right\}=0,\quad s\in I.\]
Then, according to Lemma \ref{lem:1}, we obtain \eqref{eq:suma0}.

If  \eqref{eq:suma0} is satisfied then the force field $F$ acting on the massless
