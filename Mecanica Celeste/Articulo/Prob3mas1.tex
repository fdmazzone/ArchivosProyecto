...
We note that in order to get collisionless solutions of problem $P$ we need that no primary body is located in the center of mass. We call this configurations \emph{admissible}. In the following theorem we characterize all 3-4 bodies CC  satisfying condition in Theorem \ref{thm:prim} and without bodies in the center of mass. 


\begin{thm}\label{thm:caracterizacion}
The only 3-body admissible CC is the equilateral triangle with three equal masses. In the case of 4-body, an admissible CC has two pairs of equal masses ($m_1=m_2$ and $m_3=m_4$), and satisfies some of the following properties: it is collinear and symmetric around the center of mass or it is a rhombus with the equal masses in opposite vertices.
\end{thm}
