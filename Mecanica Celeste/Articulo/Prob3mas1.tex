...
\begin{equation}\label{eq:eq_new_red}
 \ddot{z}=-\sum_{i=1 }^n\frac{m_iz}{(r_i^2+z^2)^{\frac32}}.
\end{equation}
...
\begin{thm}\label{thm:prin_ine} We assume that $q_1,\ldots,q_n$ is a admisible CC. A sufficient condition for that the problem $(P)$ has sotutions is that
\begin{equation}\label{eq:ine_prin}
 \sum_{i<j}\frac{m_im_j}{r_{ij}}<\left(\sum_{i=1}^n\frac{m_i}{|q_i|^3}\right)\left(\sum_{i=1}^nm_i|q_i|^2\right).
\end{equation}
...
\begin{proof}[Proof Theorem \ref{thm:prin_ine}] We follows a similar argument that \cite{zhao2015nonplanar}. We prove the existence of solutions by means of variational techniques.
