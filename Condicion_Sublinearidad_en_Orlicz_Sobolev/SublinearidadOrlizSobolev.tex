...
\right.
\end{equation}
where $\mathcal{L}:[0,T]\times\rr^d\times\rr^d\to\rr$, $d\geq 1$, is called the \emph{Lagrange function} or \emph{lagrangian} and the unknown function  $u:[0,T]\to\rr^d$ is absolutely continuous. In other words, we are interested in  finding \emph{periodic weak solutions} of \emph{Euler-Lagrange system of ordinary equations}. 
...
In the paper \cite{ABGMS2015} it was also assumed  a condition of \emph{bounded oscillation} on $F$  (see \cite{ABGMS2015},\cite{zhu2012analysis}). 
...
Namely, the power function is replaced by a more general one  $\Phi_0$ and 
...
And, we present the result here since the lemma introduces a function $\Phi^*$ that will play a significant role in our main theorem.
...
on the subspace $\wphie([0,T],\rr^d)$. We observe that the condition \ref{item:condicion_a} on 
the lagrangian $\mathcal{L}_{\Phi,F}$  is equivalent to say that $\mathcal{L}_{\Phi,F}\in \mathfrak{A}(a,b,0,1,0,\Phi)$.
...
\lim_{|x|\to\infty}\frac{\int_{0}^{T}F(t,x)\ dt}{\Psi_2(\Phi_0'(2|x|))}=+\infty,\tag{$A_6$}
\end{equation}
for some $N$-function $\Psi_2$ with complementary function $\Phi_2$ satisfying $\Phi_0\llcurly \Phi_2\llcurly \Phi^*$.
\end{enumerate}
...
From \cite[Thm. 2 (b)(v), p. 16]{rao1991theory}, there exists an $N$-function $\Phi_0$ such that $\Phi_0\llcurly \Phi^*$ 
and we get
...
By \eqref{eq:caso-delta-2}, there exists an $N$-function $\Phi_0$ satisfying  \eqref{eq:coer_mod} 
with the $N$-function $\Phi_1$ instead of $\Phi$. 
...
Next, we give the proof of Theorem \ref{coercitividad-r}.  Here,  we will amend a mistake that was made in the end of the proof of  \cite[Thm. 6.2]{ABGMS2015},  where it was wrongly assumed that  the minimum of  $I$ was on the domain of differentiability of $I$. We avoid this problem minimizing $I$ on $\wphiet$, which is a weak* closed subspace  contained in that domain.
...
One of the main novelties of our work is that we obtain existence of solutions for lagrangian functions $\mathcal{L}(t,x,y)$ that do not satisfy a power-like grow condition in $y$.   
...
Therefore $\Phi_0\llcurly \Phi_1\llcurly \Phi^*$.
