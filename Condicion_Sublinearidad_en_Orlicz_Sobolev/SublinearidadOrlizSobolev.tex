...
\Phi(rs)\geq \Phi^*(r)\Phi(s)\;\;\mbox{for every}\;\;r\geq1,\;\;s\geq 0.
\end{equation}
\end{enumerate}
\end{lem}
...
\]
Now, taking  $x\geq \max\{8,\Psi_1^{-1}(\Phi^*(1))\}$ we get that $\Psi \in \Delta_2$ globally.
\end{proof}

The following lemma generalizes \cite[Lemma 5.2]{ABGMS2015}.

\begin{lem}\label{lem_coer}
Let $\Phi,\Psi$ be complementary $N$-functions with $\Psi \in \Delta_2$. Then there exists a $N$-function $\Phi^*$ such that
\begin{equation}\label{eq:coer_mod}
\lim\limits_{\|u\orlnor\to \infty}
\frac{\int_0^T \Phi(|u|)\,dt}{\Phi_0(k\|u\orlnor)}=\infty,
\end{equation}
for every $\Phi_0$ with $\Phi_0\llcurly\Phi^*$ and $k>0$. Reciprocally, if  \eqref{eq:coer_mod} holds for some $N$-function $\Phi_0$,  then $\Psi\in\Delta_2$.
\end{lem}
...
=\infty.
\]
 If $\Psi$ is $\Delta_2$, but it is not $\Delta_2$-globally, we  use Lemma \ref{lem:delta2-equiv-delta2-global}. Then there exists a $Delta_2$-globally $N$-function $\Psi^*$, with $\Psi^*\equiv\Psi\leq \Psi^*$. Let $\Phi^*$ the complementary function of $\Psi^*$. 
