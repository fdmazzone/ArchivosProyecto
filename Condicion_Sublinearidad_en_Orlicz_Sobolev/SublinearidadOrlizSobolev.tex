...
ESCRIBIR ALGO SOBRE EL RESULTADO PRINCIPAL!!!!
...
\subsection{More general action integrals}


Muuuuuy en construcci\'on.....Fernando: no s\'e si era la idea que ten\'ias en mente..... 
...
\right.
\end{equation}
where $T>0$, $\b{u}:[0,T]\to\rr^d$ is absolutely continuous and 
the \emph{Lagrangian} $\mathcal{L}:[0,T]\times\rr^d\times\rr^d\to\rr$ is a Carath\'eodory and differentiable function satisfying the conditions
\begin{eqnarray}
...
\end{eqnarray}
and where the potential $F$ satisfies conditions (A) and (C).

In these inequalities we assume that  $a\in C(\mathbb{R}^+,\mathbb{R}^+)$, $\lambda>0$, $\Phi$ is an $N$-function (see section  Preliminaries  for definitions), $\varphi$ is the right continuous derivative of $\Phi$. The non negative functions $b,c$ and $f$ satisfy that  $b\in L^1_1([0,T]) $,  $c\in\lpsi_1([0,T])$ and  $f\in \ephi_1([0,T])$, where  the Banach spaces $ L^1_1([0,T]), \lpsi_1([0,T])$ and  $\ephi_1([0,T])$.

Now,  performing some slight modifications in the proof of Theorem \ref{coercitividad-r}
(in first line of \eqref{cota_inf_I}, $=$ has to be changed by $\geq$ and  
Thm. 4.1 of  \cite{ABGMS2015} can be applied to prove $u'(0)=u'(T)$ exchanging $\wphi$ by $\wphiet$),
we also get that problem \eqref{ProbPrin-gral} has at least one solution
...
\end{equation}
 ...
 \section{Growing conditions with power functions}


As it has been seen in the proof of Lemma \ref{lem:submultipliativa} and in \cite[Lemma 5.2]{ABGMS2015}, the function $\Phi_0$ in Lemma \ref{lem:submultipliativa} may be assumed a power function $\Phi_0(x)=|x|^{\mu}$ with $0<\mu<\alpha_{\Phi}$ and  $\alpha_{\Phi}$  is a Matuszewska-Orlicz indices (see \cite[Ch 11]{M}). These indices are defined by
...
\end{equation}
Hence, following the same lines that in the proof of Theorem \ref{coercitividad-r},  using \cite[Lemma 5.2]{ABGMS2015} instead of our Lemma \ref{lem:submultipliativa}, we can assume that $\Phi_0(x)=|x|^{\mu}$ with $0<\mu<\alpha_{\Phi}$ in  Theorem \ref{coercitividad-r}.




 The employment of  $N$-functions instead of power functions in  inequalities like  \eqref{holder_cont-mu}  will allow us to extend some results of   \cite{tang1998periodic} and \cite{tang2010periodic}, no only to the $\Phi$-laplacian operator, but even in the case of  $p$-laplacian operator we get bounds that may be more sharp that those in \cite{tang1998periodic,tang2010periodic}. More precisely, in  \cite[Th. 2.1]{tang2010periodic} X. Tang and X. Zhang obtained existences of solutions for the $p$-laplacian operator under la assumption
