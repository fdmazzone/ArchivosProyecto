...
Now, taking into account the relations $\left[L^1\right]^*=L^{\infty}\subset  \epsi$ and $\lphi\subset L^1$, we have that $u'_n$ converges to $u'$ in the weak topology of $L^1$. Consequently,  the semicontinuity lemma (see \cite[Lemma 6.1]{ABGMS2015})  implies that
\[I(u)\leq  \liminf_{n\to\infty}I(u_n)=\inf\limits_{u\in\wphie_T}I(u).\]

Hence $u\in \wphiet$ is a minimun and since $I$ is G\^ateaux differentiable on $\wphie$ (see  \cite[Th. 3.2]{ABGMS2015}) we have
and therefore  $I'(u)\in (\wphiet)^{\perp}$. Therefore
\[\int_0^T \frac{\Phi'(|u'(t)|)}{|u'(t)|}u'(t)\cdot v'(t)dt =-\int_0^T \nabla F(t,u(t))\cdot v(t)dt,\]
for every  $v\in \wphiet$.  Since $\frac{\Phi'(|u'(t)|)}{|u'(t)|}u'(t)\in \lpsi([0,T],\rr^n)\hookrightarrow L^1([0,T],\rr^n)$ and  $\nabla F(t,u(t))\in L^1([0,T],\rr^n)$.



Then, from \cite[Lemma 2.4]{ABGMS2015} we obtain
 $u'(t)\Phi'(|u'(t)|)/|u'(t)|\in \lpsi([0,T],\rr^n)\hookrightarrow L^1([0,T],\rr^n)$ and  from condition (A) and $u\in L^{\infty}$ we have $\nabla F(t,u(t))\in L^1([0,T],\rr^n)$. Consequently, from \cite[p. 6]{mawhin2010critical} we obtain that the differential equations in \eqref{ProbPrin} is verified and $u'(0)\Phi'(|u'(0)|)/|u'(0)|=u'(T)\Phi'(|u'(T)|)/|u'(T)|$ holds. Thus $u'(0)=u'(T)$.
\end{proof}
