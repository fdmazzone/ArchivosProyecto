...
Lo que sigue estaba en el trabajo anterior!!!!
...
EL SIGUIENTE LEMA DEBER\'IA ADAPTARSE CON $\ephi_T$ o sirve igual as\'i???????
...
$u=s\in S$.
As $\epsi$ is separable and $\lphi=\left[\epsi\right]^*$, the ball of $\lphi$ $\{u \in \lphi | \|u\orlnor\leq 1\}$ is weak* metrizable (see \cite[Thm. 5.1, p. 138]{}).
Thus, $S$ with the weak * topology. Now, \cite[Cor. 12.6, p. 165]{} implies that $\ephi_d$ is weak* closed.
\end{proof}
...
\begin{proof} First of all, note that  \eqref{cota_inf}, \eqref{condA1} and \eqref{propiedad1coercividad} imply that $I$ is, lower bounded on $\wphi_T$. Let $\{\b{u}_n\}\subset \wphiet$ be a  minimizing sequence for the problem  $\min\{I(\b{u})|\b{u}\in\wphiet\}$.
