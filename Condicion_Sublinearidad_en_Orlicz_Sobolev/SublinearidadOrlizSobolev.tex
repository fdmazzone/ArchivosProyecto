...
\begin{equation}\label{holder_cont-phi0}
...
Besides, $\ephi$ is separable (see \cite[Thm. 1, p. 87]{rao1991theory}).
...
Thus, $S$ with the weak * topology. Now, \cite[Cor. 12.6, p. 165]{Conway1977} implies that $\ephi_d$ is weak* closed.
\end{proof}
...
Suppose that the differentiable Carath\'eodory function $\mathcal{L}(t,\b{x},\b{y})$ is strictly convex at $\b{y}$, $D_{\b{y}}\mathcal{L}$ is $T$-periodic with respect to $T$  and \eqref{cotaL}, \eqref{cotaDxL}, \eqref{cotaDyL}, \eqref{cota_inf}, \eqref{condA2} and \eqref{eq:propiedad-coercividad-phi0} are satisfied. In addition, assume that some of the following statements hold (we recall the definitions and properties of $\alpha_0$, $b_1$, $\b{x}_0$ and $b_0$ from \eqref{cota_inf}, \eqref{holder_cont-phi0}, \eqref{der_cero} and \eqref{condA2} respectively):
\begin{enumerate}
  \item\label{item1prin} $\Psi\in\Delta_2$ and  \eqref{holder_cont-phi0}.
\item \eqref{holder_cont-phi0} and  $\alpha_0^{-1}T\Phi^{-1}\left(1/T\right)\|b_1\|_{L^1}\Lambda<1$.
...
\begin{proof} First of all, note that  \eqref{cota_inf}, \eqref{condA2} and \eqref{eq:propiedad-coercividad-phi0} imply that $I$ is, lower bounded on $\wphi_T$. Let $\{\b{u}_n\}\subset \wphiet$ be a  minimizing sequence for the problem  $\min\{I(\b{u})|\b{u}\in\wphiet\}$.
Since  $I(\b{u}_n)$, $n=1,2,\ldots$  is bounded, Theorem \ref{coercitividad1} (or Theorem \ref{segcoerthm} according to which of the items \ref{item1prin}-\ref{item4prin} hold true) implies that $\{\b{u}_n\}$ is norm bounded in $\wphie_d$. Hence, in virtue of Corollary \ref{unif_conv}, we can assume that $\b{u}_n$ converges uniformly to a $T$-periodic continuous function $\b{u}$.  
