...
The \emph{Orlicz class} $C_d^{\Phi}=C_d^{\Phi}([0,T],\rr^d)$  is defined  by
...
A generalized version of \emph{H\"older's inequality} holds in Orlicz spaces (see \cite[Thm. 9.3]{KR}). Namely, if $u\in\lphi$ and $v\in\lpsi$ then $u\cdot v\in L_1^1$ and
...
\]
Now, we choose $r=\frac{\|u\orlnor}{2}$ and as $\|u\orlnor\to\infty$ we can assume $r>1$. And, as
...
=\infty.
\]
The last equality follows from \cite[Thm. 2 (b), p. 16]{rao1991theory}.
...
where  $b_1,b_2 \in L^1_1$ and $\alpha$ is any power less than $p$. Thus, they called $F$  a sublinear nonlinearity. 
...
-C_4 
\\&=
\alpha_0J_{C_6,\Phi_0}\left(\frac{u'}{\Lambda}\right)
+H_{C_4, \Phi_0}(\overline{u})
-C_4, 
\end{split}
\end{equation}
...
Now, Lemma \ref{lem_coer} implies that the functional $J_{C_6,\Phi_0}(\frac{u'}{\Lambda})$ is coercive;
and, by \eqref{eq:propiedad-coercividad-phi0},
the functional $H_{C_4,\Phi_0}(\overline{u})$ is also coercive, then
$J_{C_6,\Phi_0}(\frac{u'_n}{\Lambda}) \to \infty$ or $H_{C_4,\Phi_0}(\overline{u}_n)\to \infty$.
From the condition (A) on $F$, we have that on a bounded set the functional $H_{C_4,\Phi_0}(\overline{u}_n)$ is lower bounded and also $J_{C_6,\Phi_0}(\frac{u'_n}{\Lambda})\geq 0$.
...
\[
\int_0^T\b{\xi'}\cdot u\ dt=\lim_{n\to\infty}\int_0^T\b{\xi'}\cdot u_n \ dt=
-\lim_{n\to\infty}\int_0^T\b{\xi}\cdot u'_n\ dt=-\int_0^T\b{\xi}\cdot v\ dt
\]
...
????? est\'a repetido????
Since $\frac{\Phi'(|u'(t)|)}{|u'(t)|}u'(t)\in \lpsi([0,T],\rr^n)\hookrightarrow L^1([0,T],\rr^n)$ and  $\nabla F(t,u(t))\in L^1([0,T],\rr^n)$.
