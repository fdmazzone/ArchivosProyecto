...
(see \cite[p.84]{M}). The relation $\beta_{\eta}<+\infty$ holds true if and only if $\eta$ is a
$\Delta_2$-function (\cite[Theorem 11.7]{M}). If $(\Phi,\Psi)$ is a complementary pair of  $N$-functions then
\begin{equation}\label{compl_ind}
 \frac{1}{\alpha_{\Phi}}+\frac{1}{\beta_{\Psi}}=1,
\end{equation}
(see \cite[Corollary 11.6]{M}). Therefore $1\leq \alpha_{\Phi}\leq\beta_{\Phi}\leq \infty $.

 If $\eta$ is an increasing $\Delta_2$-function then $\eta$ is controlled by above and below
 by power functions (\cite[Section 1]{Gustavsson1977}, \cite[Equations 2.3-2.4]{fiorenza1997indices} and \cite[Theorem 11.13]{M}).   More concretely, for every $\epsilon>0$ there exists a
...
As consequences of the $\Delta_2$-condition on $\Psi$, \eqref{compl_ind} and \eqref{delta2-potencias} we get for every $1<\nu<\alpha_{\Phi}$ a constant $K=K_{\nu}>0$  such that
...
\end{split}
\end{equation}
where $C_2=C_2(T, \|b_1\|_{L^1} )$ and $C_3=C_3(T)$. Next, by Young's inequality with complementary functions $\Phi_1$ and $\Psi_1$
...
for every  $v\in \wphiet$.  From \cite[Eq. (26)]{ABGMS2015} we have
 $D_y\mathcal{L}(t,u(t),u'(t))\in \lpsi([0,T],\rr^n)\hookrightarrow L^1([0,T],\rr^n)$;
and,  from  \cite[Eq. (24)]{ABGMS2015}, it follows that $D_x\mathcal{L}(t,u(t),u'(t))\in L^1([0,T],\rr^n)$. Consequently, from \cite[p. 6]{mawhin2010critical} (note that $\wphiet$  include the periodic test functions) we obtain the absolutely continuity of $D_y\mathcal{L}(t,u(t),u'(t))$ and that the differential equations in \eqref{ProbPrin-gral} is verified. The strict convexity of $\mathcal{L}(t,x,y)$ with respect to $y$ and the $T$-periodicity with respect to $t$ imply the boundary conditions in  \eqref{ProbPrin-gral} (see \cite[Th. 4.1]{ABGMS2015}).\qed
