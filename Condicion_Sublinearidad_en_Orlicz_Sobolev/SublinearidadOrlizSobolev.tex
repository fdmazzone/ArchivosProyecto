...
$b\in L^1_1([0,T])$,  $c\in\lpsi_1([0,T])$ and  $f\in \ephi_1([0,T])$. It was shown in
\cite{ABGMS2015} that conditions (A1-3)  guarantee the  existence of the Gate\^aux derivative $I'(u)$ of the integral functional $I$, defined in \eqref{integral_accion}, on the subspace $\wphie([0,T],\rr^d)$. We note that for the lagrangian $\mathcal{L}_{\Phi,F}$ assumptions (A1-3) are equivalent to condition (A) for the potential $F$.
It is needed  other assumptions on Lagrange function $\mathcal{L}$.
As mentioned we
