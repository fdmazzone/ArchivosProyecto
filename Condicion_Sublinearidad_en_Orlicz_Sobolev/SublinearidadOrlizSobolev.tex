...
Finally, if $\Phi_0$ is an $N$-function, then $\Phi_0(x)\geq \alpha |x|$ for  $k$ small enough and $|x|>1$.
...
We can applied Theorem \ref{coercitividad-r} to Lagrangians $\mathcal{L}=\mathcal{L}(t,x,y)$ with an exponential grow in $y$ like
  \[\mathcal{L}(t,x,y)=e^{|y|}+F(t,x),\]
In this case, considering  the $N$-function $\Phi(x)=e^x-x-1$,   whose complementary
function  $\Psi(x)=(1+x)\ln(1+x)-x$, satisfies the $\Delta_2$-condition.
...
function of $\Phi$, i.e.  $\Psi(x)=(1+x)\ln(1+x)-x$, satisfies the $\Delta_2$-condition. We can apply our main theorem even if $\Psi\notin\Delta_2$, although in this case we need an additional condition on the size of certain constants.
 ...
 Furthermore, we want to emphasize that, even in the case of $p$-laplacian operator \eqref{ProbP-lapla}, our results  extend  previous one (see \cite{tang1998periodic, tang2010periodic}), because
 ...
 We note that this $F$ does not satisfies \eqref{eq:cota_pot} for any $\alpha<p-1$.  Next we will show  a $N$-function $\Phi_0$ satisfying the hypothesis of Theorem \ref{coercitividad-r} for this potential $F_0$.
...
\begin{thm}
If $p\geq \frac{1+\sqrt 2}{2}$, then $\Phi_0$ is an differentiable $N$-function. The $N$-function $\Phi_0$ satisfies that for every $\varepsilon>0$, there exists a positive constant $C=C(p,\varepsilon)$  such that
...
$\varphi_2'(u)>0$ on $[e,\infty)$ if and only if
 ...
 Besides $\varphi_2(u)\to \infty$ and  $\varphi_1(u)\to 0$  as $u \to  \infty$ and $u\to 0$  respectively, provided that $p>1$. Hence, $\Phi_0$ is an $N$-function.
