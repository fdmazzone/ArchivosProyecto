...
\end{equation}
And, we introduce the following subset of $\wphi$
%%
\begin{equation}\label{def-esp-orlicz-sob-per}
\begin{split}
\wphi_T&=\{u\in\wphi|u(0)=u(T)\}.
\end{split}
\end{equation}
...
It is worth noting that the second improvement is more important than the first one.
...
on the set
\[\domi:=\{u\in\wphi([0,T],\rr^d): d(u',\ephi)<\lambda\}.\]
We observe that the condition \ref{item:condicion_a} on the potential $F$ is equivalent to say that $\mathcal{L}_{\Phi,F}\in \mathfrak{A}(a,b,0,1,0,\Phi)$.
...
Next, we give our main result.  Here,  we will amend an
...
and suppose that $\Phi^*$ is given by Lemma \ref{lem_coer}. Assume that the lagrangian $\mathcal{L}\in \mathfrak{A}(a,b,c,\lambda,f,\Phi)$ satisfies \eqref{eq:control_inf}, where
the potential $F$ fulfills  \ref{item:condicion_c}, \ref{item:condicion_a} and
...
Then the action integral $I$ has a minimum $u\in\wphi([0,T],\rr^d)$ such that $d(u',\ephi)\leq \lambda$. If $d(u',\ephi)< \lambda$, the lagrangian $\mathcal{L}(t,x,y)$ is strictly convex  with respect to $y\in\rr^d$ and
$D_y\mathcal{L}(0,x,y)=D_y\mathcal{L}(T,x,y)$
then $u$ solves the problem \eqref{ProbPrin-gral}.

\end{thm}



\begin{comentario} If $\Phi\in\Delta_2^{\infty}$ the the condition  $d(u',\ephi)\leq \lambda$ is automatically satisfied.
\end{comentario}
...
Let $\{u_n\}\subset \wphi$  be a  minimizing sequence for the problem
\linebreak $\inf\{I(u)|u\in\wphi\}$.
Since  $I(u_n)$, $n=1,2,\ldots$,  is upper bounded, the previous part of the proof shows that $\{u_n\}$ is norm bounded. Hence, by virtue of  \cite[Cor. 2.2]{ABGMS2015}, we can assume, taking a subsequence if necessary, that $u_n$ converges uniformly to a $T$-periodic continuous   function $u$. As $\{u'_n\}$ is a norm bounded sequence in $\lphi$,
...
Thus $v=u'$ a.e. $t\in [0,T]$ (see \cite[p. 6]{mawhin2010critical}) and $u\in\wphi([0,T],\rr^d)$.
...
\[I(u)\leq  \liminf_{n\to\infty}I(u_n)=\inf\limits_{v\in\wphi_T}I(v).\]
Hence $u\in \wphiet$ is a minimun of $I$ on $\wphi_T$.

For the second part of the theorem, assume that $u$ is a minimum of $I$ with $d(u',\ephi)<\lambda$.
Since $I$ is G\^ateaux differentiable at $u$ (see  \cite[Thm. 3.2]{ABGMS2015}),
therefore $I'(u)\in (\wphi_T)^{\perp}$. Thus,
\[\int_0^T D_y\mathcal{L}(t,u(t),u'(t))\cdot v'(t)dt =-\int_0^T D_x\mathcal{L}(t,u(t),u'(t))\cdot v(t)dt,\]
for every  $v\in \wphi_T$.
...
The main novelty  of our work is that we obtain existence of minima of $I$ associated with  lagrangian functions $\mathcal{L}(t,x,y)$ that do not satisfy a power-like grow condition on $y$.
...
In conclusion, the action integral $I$ associated with the Lagrangian $\mathcal{L}(t,x,y)=f(y)+b(t)|x|^{\sigma}$   has minimum for any $1<\sigma$.
...
\bibitem{mawhin2010critical}
J.~{M}awhin, M.~{W}illem, Critical point theory and {H}amiltonian systems,
  Springer-Verlag, New York, 1989.

\bibitem{tang1995periodic}
C.-L. Tang, Periodic solutions of non-autonomous second-order systems with
  $\gamma$-quasisubadditive potential, Journal of Mathematical Analysis and
  Applications 189~(3) (1995) 671--675.

\bibitem{tang1998periodic}
C.-L. Tang, Periodic solutions for nonautonomous second order systems with
  sublinear nonlinearity, Proc. Amer. Math. Soc. 126~(11) (1998) 3263--3270.

\bibitem{wu1999periodic}
X.-P. Wu, C.-L. Tang, Periodic solutions of a class of non-autonomous
  second-order systems, J. Math. Anal. Appl. 236~(2) (1999) 227--235.

\bibitem{tang2001periodic}
C.~L. Tang, X.-P. Wu, Periodic solutions for second order systems with not
  uniformly coercive potential, J. Math. Anal. Appl. 259~(2) (2001) 386--397.

\bibitem{zhao2004periodic}
F.~Zhao, X.~Wu, Periodic solutions for a class of non-autonomous second order
  systems, J. Math. Anal. Appl. 296~(2) (2004) 422--434.

\bibitem{Tian2007192}
Y.~Tian, W.~Ge, Periodic solutions of non-autonomous second-order systems with
  a {$p$}-{L}aplacian, Nonlinear Anal. 66~(1) (2007) 192--203.

\bibitem{tang2010periodic}
X.~Tang, X.~Zhang, Periodic solutions for second-order {H}amiltonian systems
  with a {$p$}-{L}aplacian, Ann. Univ. Mariae Curie-Sk\l odowska Sect. A 64~(1)
  (2010) 93--113.

\bibitem{ABGMS2015}
S.~{A}cinas, L.~{B}uri, G.~Giubergia, F.~Mazzone, E.~Schwindt, Some existence
  results on periodic solutions of {E}uler-{L}agrange equations in an
  {O}rlicz-{S}obolev space setting, Nonlinear Analysis, TMA. 125 (2015) 681 --
  698.

\bibitem{zhao2005existence}
F.~Zhao, X.~Wu, Existence and multiplicity of periodic solution for
  non-autonomous second-order systems with linear nonlinearity, Nonlinear
  Analysis: Theory, Methods \& Applications 60~(2) (2005) 325--335.

\bibitem{adams_sobolev}
R.~Adams, J.~Fournier, Sobolev spaces, Elsevier/Academic Press, Amsterdam,
  2003.

\bibitem{KR}
M.~A. Krasnosel{\cprime}ski{\u\i}, J.~B. Ruticki{\u\i}, Convex functions and
  {O}rlicz spaces, P. Noordhoff Ltd., Groningen, 1961.

\bibitem{rao1991theory}
M.~M. Rao, Z.~D. Ren, Theory of {O}rlicz spaces, Vol. 146, Marcel Dekker, Inc.,
  New York, 1991.

\bibitem{M}
L.~Maligranda, Orlicz spaces and interpolation, Vol.~5 of Semin\'arios de
  Matem\'atica [Seminars in Mathematics], Universidade Estadual de Campinas,
  Departamento de Matem\'atica, Campinas, 1989.

\bibitem{Gustavsson1977}
J.~Gustavsson, J.~Peetre, \href{http://eudml.org/doc/218150}{Interpolation of
  orlicz spaces}, Studia Mathematica 60~(1) (1977) 33--59.
\newline\urlprefix\url{http://eudml.org/doc/218150}

\bibitem{fiorenza1997indices}
A.~Fiorenza, M.~Krbec, Indices of {O}rlicz spaces and some applications,
  Commentationes Mathematicae Universitatis Carolinae 38~(3) (1997) 433--452.

\bibitem{hudzik2000amemiya}
H.~Hudzik, L.~Maligranda, Amemiya norm equals {O}rlicz norm in general, Indag.
  Math. (N.S.) 11~(4) (2000) 573--585.

\bibitem{xu2007some}
B.~Xu, C.-L. Tang, Some existence results on periodic solutions of ordinary
  {$p$}-{L}aplacian systems, J. Math. Anal. Appl. 333~(2) (2007) 1228--1236.

\bibitem{zhu2012analysis}
K.~Zhu, Analysis on Fock Spaces, Graduate Texts in Mathematics, Springer, 2012.

\bibitem{Conway1977}
J.~B. Conway, A Course in Functional Analysis, Springer, USA, 1977.
