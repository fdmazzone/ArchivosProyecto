...
On these assumptions, $\varphi:\mathbb{R}^+\rightarrow \mathbb{R}^+$ is a homeomorphism whose inverse will be denoted $\psi$. 
We denote by $\Psi$ the primitive of $\psi$ that satisfies $\Psi(0)=0$. Then, $\Psi$ is an $N$-function which  is called the \emph{complementary function} of $\Phi$.
 ...
 We recall that a function $N$-function $\Phi(u)$ has \emph{principal part} the function $f(u)$ if $\Phi(u)=f(u)$ for large value of the argument (see \cite[p. 16]{KR} and \cite[Sec. 7]{KR} for  properties of principal part)
...
If $x_0=0$,  the function   $\eta:\mathbb{R}^+\rightarrow \mathbb{R}^+$ is said to satisfy the $\Delta_2$ ($\eta \in \Delta_2$). If there exists $x_0>0$ such that  inequality \eqref{delta2defi} holds for $x\leq x_0$ we said that $\Phi$ satisfies the $\Delta_2^0$-condition ($\Phi\in\Delta_2^0$).



Let $d$ be a positive integer. We denote by $\mathcal{M}:=\mathcal{M}([0,T],\rr^d)$  the set of all measurable functions defined on $[0,T]$ with values on $\mathbb{R}^d$ and  we write $u=(u_1,\dots,u_d)$ for  $u\in \mathcal{M}$. For the set of functions $\mathcal{M}$, as for other similar sets, we omit the reference to codomain $\mathbb{R}^d$ when $d=1$.
...
We begin by stating the following lemma which establish the coercivity of the modular function $\rho_{\Phi}(u)$ with respect to certain functions of the Orlicz norm $\Phi_0(\|u\orlnor|)$. This lemma generalizes \cite[Lemma 5.2]{ABGMS2015} in two directions. The first, we consider function $\Phi_0$ more general than in  \cite[Lemma 5.2]{ABGMS2015}. The second one and more important, the condition $\Delta_2$-global on $\Psi$ used in \cite[Lemma 5.2]{ABGMS2015} was relaxed to $\Delta_2^{\infty}$. We need to present this result here since the lemma introduces the function $\Phi^*$ that plays an important role in our main theorem.

\begin{lem}\label{lem_coer}
Let $\Phi,\Psi$ be complementary $N$-functions with $\Psi \in \Delta_2^{\infty}$. Then there exists a $N$-function $\Phi^*$, with $\Phi^*\prec\Phi$, such that  for every $\Phi_0\llcurly\Phi^*$ and $k>0$
...
Likes \cite{ABGMS2015} we will consider general lagrangian functions
...
$\mathcal{L}_{\Phi,F}\in \mathfrak{A}(a,b,0,1,0,\Phi)$.
 ...
 Unlike what is usual in the literature, we do not assume that the Lagrangian $\mathcal{L}$  split in two terms one of them function of $y$ and the other one function of $(t,x)$. We suppose only a lower bound by a function of that type. More precisely we assume that for every $(t,x,y)\in\rr\times\rr^d\times\rr^d$
...
Moreover, as is usual, it is needed to suppose that the time integral of $F$ satisfies certain coercivity condition, see \eqref{eq:propiedad-coercividad-phi0} below.  But even all these hypotheses are not enough. It is necessary to assume extra conditions on the potential $F$.
Several hypotheses were tested in the past years. The so-called \emph{subconvexity} for $F$ was tried in \cite{wu1999periodic,tang1995periodic,zhao2004periodic} for semilinear equations and in \cite{xu2007some,tang2010periodic} for $p$-laplacian systems. A boundeness on the oscilation of $F$ was studied in \cite{ABGMS2015}. In \cite{tang1998periodic, tang2010periodic}) the authors  considered, for the $p$-laplacian case, potentials $F$ satisfying the inequality
\begin{equation}\label{eq:cota_pot} |\nabla F(t,x)|\leq b_1(t)|x|^{\alpha}+b_2(t),
 \end{equation}
where  $b_1,b_2 \in L^1([0,T])$ and $\alpha<p$. The potentials $F$ satisfying these inequalities  were called by the authors \emph{sublinear nonlinearities}. In this paper, we are interesting in to study this type of potential, but with more general bounds on $\nabla F$ which includes $N$-functions instead power functions, namely, we will consider inequalities for $F$ of the following type
...
\begin{thm}\label{coercitividad-r} Let $\Phi$ be an $N$-function, whose complementary function $\Psi$ satisfies  $\Psi\in\Delta_2^{\infty}$ and suppose that $\Phi^*$ satisfies the Lemma \ref{lem_coer}. Assume that the potential $F$ satisfies \ref{item:condicion_c} and \ref{item:condicion_a} and the conditions
...
\begin{lem}\label{lema:delta_2 y der} Let $\Phi$ be a (not needed differentiable) $N$-function and $\varphi$ the right continuous derivative of $\Phi$. Then $\Phi\in\Delta_2^{\infty}$ (
...
The following lemma improves the result at the comment on the beginning of \cite[p. 24]{KR}.

\begin{lem}\label{lem:delta2-equiv-delta2-global} Let $\Psi$ be a $N$-function satisfying the $\Delta_2^{\infty}$ condition. Then there exists a $N$-function $\Psi^*$  such that $\Psi^*\in\Delta_2$, $\Psi\leq\Psi^*$ and for every $a>1$ there exists $x_0=x_0(a)\geq 0$ such that $\Psi^*(x)\leq a\Psi(x)$, for every $x\geq x_0$.  In other words,  every $\Delta_2$ near infinity  $N$-function is equivalent to a $\Delta_2$ globally $N$-function.
\end{lem}
...
The following lemma is essentially known,  this is basically consequence  of the fact that $\Psi\in\Delta_2^{\infty}$ if and only if $\Psi\closedprec\Psi$, of  \cite[Prop. 4, p. 20]{rao1991theory} and \cite[Cor. 10, p. 30]{rao1991theory}. However we prefer included an alternative proof, because we do not see clear that the previous results of \cite{rao1991theory} contemplate the case in the following lemma of the $N$-functions satisfying the $\Delta_2$ condition globally.
...
\noindent\textbf{Proof Lemma \ref{lem_coer}} At first we assume that $\Phi$ is $\Delta_2$-globally. Let $\Phi^*$ be a $N$ function satisfying \eqref{eq:caract_delta2}. By the inequality \eqref{amemiya-ine}, for $r>1$ we have
\[
...
Next we give the proof of the Theorem \ref{coercitividad-r}.  Here we correct an error in the end of the proof of  \cite[Thm. 6.2]{ABGMS2015}  where was wrongly assumed that  the minimum of  $I$ is in the domain of differentiability of $I$. We avoid this problem by minimizing $I$ on $\wphiet$ which is a subspace weak* closed contained that domain.
...
and,  from  \cite[Eq. (24)]{ABGMS2015}, it follows that $D_x\mathcal{L}(t,u(t),u'(t))\in L^1([0,T],\rr^n)$. Consequently, from \cite[p. 6]{mawhin2010critical} we obtain that the differential equations in \eqref{ProbPrin-gral}
are verified.
...
Now we consider a genral lagragian $\mathcal{L}$.
...
the function $D_y\mathcal{L}(T,u(T),\cdot)$ imply that $u'(0)=u'(T)$.\qed
