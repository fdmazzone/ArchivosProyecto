...
\[
  \begin{split}
    \b{r}&=(\b{r}_1,\ldots,\b{r}_N)\in\rr^{dN},\quad\text{Vector configuración},\\
...
\[\boxed{M\b{r}''(t)=\nabla U(\b{r})},\quad \text{(Ecuación $N$-cuerpos)}\]
...
\end{frame} 
 ...
 \begin{frame}{ \includegraphics[scale=.5]{Noether.jpg} Simetrías e integrales primeras}
 \begin{itemize}
  \item<+-> \textbf{Simetría por traslaciones} ($\b{r}_j\rightarrow \b{r}_j+\b{v}_0t+\b{c}_0$, $\b{c}_0,\b{v}_0\in\rr^d$) $\Rightarrow$ conservación $\b{p}$  $\Rightarrow$ movimiento rectilineo uniforme de $\b{c}$ 
    \item<+-> \textbf{Simetría por rotaciones} ($\b{r}_j\rightarrow Q\b{r}_j$, $Q\in O(d)$) $\Rightarrow$ conservación $\omega$   
