...
\begin{defi}
 Let $\phih$ a $G$-function defined in the symplectic manifold $\rr^{2n}$. We say that  $\phih$ is a \emph{symplectic $G$-function} if
...
We note that if $\phih$ is symplectic then
...
As a consequence of \eqref{eq:J-phi1}, the matrix $J$ induce a isometry  between the spaces $L^{\phih}([0,T],\rr^{2d})$ and $L^{\phihe}([0,T],\rr^{2d})$.  Therefore we candefine  a bilinear form $\overline{\Omega}$  on $L^{\phih}([0,T],\rr^{2d})$ of the following way
